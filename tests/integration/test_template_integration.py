...
<body>
    {{ content }}
        ...
        template_file = self._create_test_template("basic", template_content)
    ...
    def test_load_template_with_includes(self):
...
{% endblock %}"""
        ...
        self._create_test_template("base", base_template)
        template_file = self._create_test_template("with_includes", main_template)
...
<p>著者: {{ author }}</p>
<p>日付: {{ date }}</p>"""
        template_file = self._create_test_template("variables", template_content)

        # テンプレートを読み込み
        template = self.template_manager.get_template(template_file.name)

        # 変数を展開
        test_data = self._create_test_content()
...
<p>説明はありません</p>
{% endif %}"""
        template_file = self._create_test_template("conditional", template_content)
...
{% endfor %}
        ...
        template_file = self._create_test_template("loop", template_content)
    ...
    def test_filter_expansion(self):
        """フィルター展開テスト"""
        template_content = """<h1>{{ title|upper }}</h1>
<p>内容: {{ content|truncate(50) }}</p>
<p>日付: {{ date|default("未設定") }}</p>"""
        template_file = self._create_test_template("filter", template_content)
...
<body>
    <div class="scenario">
        ...
        template_file = self._create_test_template("complete", template_content)
    ...
    def test_css_integration_output(self):
        """CSS統合出力テスト"""
        template_content = """<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <div class="footer">
        ...
        template_file = self._create_test_template("css_integration", template_content)
    ...
    def test_javascript_integration_output(self):
        """JavaScript統合出力テスト"""
        template_content = """<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <div id="details" style="display: none;">
        ...
        template_file = self._create_test_template("js_integration", template_content)
