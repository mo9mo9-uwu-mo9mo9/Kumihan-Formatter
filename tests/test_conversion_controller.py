...
from .test_base import BaseTestCase, create_test_kumihan_content
...
class TestConversionController(BaseTestCase):
    ...
    def test_convert_file_file_not_exists(self):
        ...
        assert (
            "入力ファイルが見つかりません"
            in self.mock_view.show_error_message.call_args[0][1]
        )
    ...
    def test_convert_file_success(self, mock_thread_class):
        ...
        assert (
            mock_thread_class.call_args[1]["target"] == controller._convert_file_thread
        )
    ...
    def test_generate_sample_success(self, mock_thread_class):
        ...
        mock_thread_class.assert_called_once()
        assert (
            mock_thread_class.call_args[1]["target"]
