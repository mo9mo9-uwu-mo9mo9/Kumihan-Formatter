...
class ArchitectureValidator:
    ...
    def _validate_file(self, file_path: Path) -> None:
        ...
        try:
            ...
            try:
                ...
                self._add_violation(
                    file_path, e.lineno or 1, ViolationSeverity.ERROR,
                    "syntax_error", f"構文エラー: {e.msg}"
                )
                
        except Exception as e:
            self._add_violation(
                file_path, 1, ViolationSeverity.ERROR,
                "file_read_error", f"ファイル読み込みエラー: {e}"
            )
    ...
    def _check_file_size(self, file_path: Path, lines: List[str]) -> None:
        ...
        if line_count > self.MAX_FILE_LINES:
            self._add_violation(
                file_path, 1, ViolationSeverity.CRITICAL,
                "file_too_large",
            ...
            self._add_violation(
                file_path, 1, ViolationSeverity.WARNING,
                "file_size_warning",
    ...
    def _check_function_size(self, file_path: Path, node: ast.FunctionDef, lines: List[str]) -> None:
        """関数サイズチェック"""
        ...
        if func_lines > self.MAX_FUNCTION_LINES:
            self._add_violation(
                file_path, node.lineno, ViolationSeverity.ERROR,
                "function_too_large",
                f"関数 '{node.name}' が制限を超過: {func_lines}行 > {self.MAX_FUNCTION_LINES}行",
                f"関数を小さな機能に分割してください"
            )
    
    def _check_class_size(self, file_path: Path, node: ast.ClassDef, lines: List[str]) -> None:
        """クラスサイズチェック"""
        if not hasattr(node, 'lineno') or not hasattr(node, 'end_lineno'):
            return
    ...
    def _check_function_complexity(self, file_path: Path, node: ast.FunctionDef) -> None:
        ...
        if complexity > self.MAX_COMPLEXITY:
            self._add_violation(
                file_path, node.lineno, ViolationSeverity.WARNING,
                "high_complexity",
    ...
    def _add_violation(self, file_path: Path, line_number: int, severity: ViolationSeverity,
                      rule_name: str, message: str, suggestion: Optional[str] = None) -> None:
        """違反を記録"""
    ...
    def generate_report(self) -> str:
        ...
        # 詳細
        for severity in [ViolationSeverity.CRITICAL, ViolationSeverity.ERROR, ViolationSeverity.WARNING]:
            if severity not in by_severity:
                continue
...
def main():
    """メイン実行"""
    parser = argparse.ArgumentParser(description="Kumihan-Formatter アーキテクチャバリデーター")
