...
def check_file_size(file_path: Path, max_lines: int, warning_lines: int) -> Tuple[bool, int, str]:
    """
    ファイルサイズをチェック
...
def main():
    ...
    parser.add_argument("files", nargs="*", help="チェックするファイル (未指定時は全Pythonファイル)")
    parser.add_argument("--max-lines", type=int, default=300,
                       help="最大行数制限 (デフォルト: 300)")
    parser.add_argument("--warning-lines", type=int, default=240,
                       help="警告行数 (デフォルト: 240)")
