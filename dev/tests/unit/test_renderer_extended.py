    ...
    def complex_document_data(self):
        ...
        return {
            ...
            "rooms": [
                {
                    "name": "書斎",
                    "description": "古い本が置かれた部屋"
                },
                {
                    "name": "地下室",
                    "description": "暗く湿った地下空間"
                }
            ],
            "items": [
                {
                    "name": "古い鍵",
                    "description": "地下室の扉を開ける"
                }
            ]
        }
...
<body>
    ...
    <div class="content">
        ...
        # カスタムテンプレートでレンダリング
        try:
            result = renderer.render(complex_document_data, template_path=str(custom_template))
    ...
    def test_html_escaping(self):
        ...
        try:
            ...
            if hasattr(formatter, 'escape') or hasattr(formatter, 'format'):
                escape_method = getattr(formatter, 'escape', None) or getattr(formatter, 'format', None)
...
class TestRendererPerformance:
    ...
    def test_large_document_rendering(self, renderer):
        ...
        assert rendering_time < 15.0, f"レンダリング時間が長すぎます: {rendering_time}秒"
    ...
    def test_repeated_rendering(self, renderer):
        ...
        assert average_time < 0.5, f"平均レンダリング時間が遅いです: {average_time:.3f}秒"
