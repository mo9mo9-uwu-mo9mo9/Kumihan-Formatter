...
class TestBasicWorkflow:
    ...
    def test_macos_command_conversion(self, test_workspace: Path, sample_test_file: Path):
        ...
        # 実行結果の検証（セットアップ不完了エラーはスキップ）
        if result.returncode != 0 and "Setup not completed" in result.stdout:
            pytest.skip("Command file requires setup in test environment")
    ...
    def test_drag_and_drop_simulation(self, test_workspace: Path, sample_test_file: Path):
        ...
        # 実行結果の検証（セットアップ不完了エラーはスキップ）
        if result.returncode != 0 and "Setup not completed" in result.stdout:
            pytest.skip("D&D executable requires setup in test environment")
