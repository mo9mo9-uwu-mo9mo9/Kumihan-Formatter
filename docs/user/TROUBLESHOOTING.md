...
# 手動でブラウザーを開く
open dist/output.html      # macOS
start dist\output.html     # Windows
xdg-open dist/output.html  # Linux
```
...
**macOS:**
1. システム環境設定 → 一般 → デフォルトのWebブラウザ

##### **Step 3: プレビュー無効化での実行**
```bash
# プレビューを無効にして変換
python -m kumihan_formatter.cli sample.txt --no-preview
```
...
❌ 間違った記法:
;;;画像 alt=マップ
images/map.jpg  # フォルダ名は不要
;;;

;;;画像
./images/map.jpg  # 相対パス指定は無効
;;;
```

##### **Step 3: 対応画像形式の確認**
**対応形式:** JPG, JPEG, PNG, GIF, WebP, SVG
**推奨形式:** PNG（高品質）, JPG（容量重視）
...
```bash
# ファイルサイズ確認
ls -lh ファイル名.txt    # macOS/Linux
dir ファイル名.txt       # Windows

# 行数確認
wc -l ファイル名.txt     # macOS/Linux
find /c /v "" ファイル名.txt  # Windows
```
...
# 個別変換
python -m kumihan_formatter.cli chapter1.txt -o dist/chapter1/
python -m kumihan_formatter.cli chapter2.txt -o dist/chapter2/
python -m kumihan_formatter.cli chapter3.txt -o dist/chapter3/
```
...
```bash
# 軽量モード（予定機能）
python -m kumihan_formatter.cli large_file.txt --streaming --no-preview
```
...
# Windows（PowerShell）
(Get-Content ファイル名.txt) | Set-Content ファイル名.txt
```

##### **Step 2: 長い行の確認と分割**
```bash
# 最長行の確認
awk '{print length, NR, $0}' ファイル名.txt | sort -nr | head -5  # macOS/Linux
...
```bash
# HTMLファイルを直接確認
cat dist/output.html | head -50   # macOS/Linux
type dist\output.html | more      # Windows
```
