...
Kumihan-Formatterは、文書内の見出し（見出し1〜5）から自動的に目次を生成します。

**自動生成条件**:
- 文書内に見出し1〜5のいずれかが存在する場合
- HTML出力時に自動的に`<nav class="toc">`要素として生成
- フォーマット後のファイル先頭に目次セクションが自動挿入

**目次の仕様**:
- **階層構造**: 見出しレベルに応じて入れ子構造（1〜5レベルのネスト）
- **アンカーリンク**: 各見出しへの自動リンク生成
- **ID自動付与**: 見出しに自動的にIDが付与される
...
<nav class="toc">
  <ul>
    <li><a href="#heading-1">第1章 概要</a></li>
    <li><a href="#heading-2">第2章 詳細
      <ul>
        <li><a href="#heading-3">2.1 基本機能</a></li>
        <li><a href="#heading-4">2.2 応用機能</a></li>
      </ul>
    </li>
  </ul>
</nav>
...
**注意事項**:
- 手動での目次記法は完全に廃止されました
- 目次の表示・非表示はHTML/CSS側で制御
- 見出しが存在しない文書では目次は生成されません
...
**α-dev統一により廃止された記法**:
- **手動目次記法**: `#目次#
##` - Issue #799対応により自動生成に移行
- **単一行記法**: `#キーワード# 内容` - ブロック記法統一により廃止
- **alt属性**: `alt=説明文` - Phase 1完了により削除
...
# 廃止された記法のテストケース（参考）
# 以下は廃止された記法のテストケースです（α-dev統一により削除）
...
### 9.2 特殊記法（後日実装予定）
- 脚注記法: `# 脚注 #内容##` （ブロック記法統一対応・Issue #726実装済み）
...
### 10.2 後方互換性
- **α-dev**: 破壊的変更・後方互換性完全破棄
- **v2.x以前**: 全て無効化・即時移行必須
- **α-dev以降**: 厳格なブロック記法のみサポート

## 15. 改訂履歴

| バージョン | 日付 | 変更内容 | 作成者 |
|------------|------|----------|--------|
| α-dev-799 | 2025-08-05 | Issue #799対応: 目次完全自動生成機能実装 - 手動目次記法（#目次#）完全廃止、目次自動生成仕様追加 | Claude Code |
