...
4. **コミット後** に *プルリクエスト* を作成する
5. **重要**: PRを作成したら **必ず自動マージを有効化** する
   ```bash
   # PR作成と同時に自動マージを有効化（推奨）
   gh pr create --title "タイトル" --body "説明" && gh pr merge --auto --squash
   
   # または既存のPRに対して
   gh pr merge [PR番号] --auto --squash
   ```
6. **マージ後**、関連 **Issue を更新・整理** する
7. マージが完了したら **不要ブランチを削除** する
8. 1から7までの作業完了後、作業に関連するIssueがあれば更新する

### 🤖 自動マージの利点
- **開発効率向上**: 手動マージ待ちの時間削減
- **一貫性保証**: CIが通った場合のみマージ実行
- **作業フロー改善**: 次のタスクにすぐ移行可能
- **自動更新対応**: PRがmainより遅れていても自動で更新・マージ
