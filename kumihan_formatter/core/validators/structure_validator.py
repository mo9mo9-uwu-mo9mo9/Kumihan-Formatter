...
class StructureValidator:
    ...
    def validate_ast_structure(self, ast: List[Node]) -> List[ValidationIssue]:
        ...
        for issue in ast_issues:
            issues.append(
                ValidationIssue(level="error", category="structure", message=issue, code="INVALID_AST_STRUCTURE")
            )
    ...
    def validate_toc_structure(self, ast: List[Node]) -> List[ValidationIssue]:
        ...
        for issue in toc_issues:
            issues.append(
                ValidationIssue(level="warning", category="structure", message=issue, code="INVALID_TOC_STRUCTURE")
            )
    ...
    def validate_document_structure(self, lines: List[str]) -> List[ValidationIssue]:
        ...
        if not lines or all(not line.strip() for line in lines):
            issues.append(
                ValidationIssue(
                    level="warning", category="structure", message="Document is empty", code="EMPTY_DOCUMENT"
                )
            )
            return issues
        ...
        for node in ast:
            if hasattr(node, "type") and node.type == "heading":
                if hasattr(node, "level") and hasattr(node, "content"):
                    entries.append({"level": node.level, "text": node.content, "id": getattr(node, "id", None)})
        return entries
    ...
    def _validate_heading_hierarchy(self, heading_levels: List[tuple]) -> List[ValidationIssue]:
        """Validate heading hierarchy"""
        issues = []
