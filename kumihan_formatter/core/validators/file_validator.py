...
class FileValidator:
    ...
    def validate_file_path(self, file_path: Path) -> List[ValidationIssue]:
        ...
        if not file_path.exists():
            issues.append(
                ValidationIssue(
                    level="error", category="file", message=f"File not found: {file_path}", code="FILE_NOT_FOUND"
                )
            )
            return issues
        ...
        if not file_path.is_file():
            issues.append(
                ValidationIssue(
                    level="error", category="file", message=f"Path is not a file: {file_path}", code="NOT_A_FILE"
                )
            )
            return issues
        ...
        try:
            ...
            issues.append(
                ValidationIssue(
                    level="error", category="file", message=f"Permission denied: {file_path}", code="PERMISSION_DENIED"
                )
            )
            ...
            issues.append(
                ValidationIssue(
                    level="error", category="file", message=f"Cannot read file: {str(e)}", code="FILE_READ_ERROR"
                )
            )
    ...
    def validate_output_path(self, output_path: Path) -> List[ValidationIssue]:
        ...
        try:
            ...
            issues.append(
                ValidationIssue(
                    level="error", category="file", message=f"Output path error: {str(e)}", code="OUTPUT_PATH_ERROR"
                )
            )
