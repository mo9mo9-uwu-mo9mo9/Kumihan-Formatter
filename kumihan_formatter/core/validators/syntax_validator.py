...
class SyntaxValidator:
    ...
    def validate_marker_syntax(self, lines: List[str]) -> List[ValidationIssue]:
        ...
        for i, line in enumerate(lines):
            ...
            if line.strip().startswith(";;;"):
                is_valid, error_messages = self.marker_validator.validate_marker_line(line.strip())
                if not is_valid:
    ...
    def validate_list_syntax(self, lines: List[str]) -> List[ValidationIssue]:
        ...
        for msg in list_issue_messages:
            issues.append(ValidationIssue(level="warning", category="syntax", message=msg, code="LIST_STRUCTURE"))
        return issues
    ...
    def validate_block_syntax(self, lines: List[str]) -> List[ValidationIssue]:
        ...
        for msg in block_issue_messages:
            issues.append(ValidationIssue(level="error", category="syntax", message=msg, code="BLOCK_STRUCTURE"))
        return issues
