...
class ValidationReporter:
    ...
    def _generate_text_report(self, issues: List[ValidationIssue]) -> str:
        ...
        for level, items, icon in [("ERRORS", errors, "❌"), ("WARNINGS", warnings, "⚠️ "), ("INFO", info, "ℹ️ ")]:
            if items:
    ...
    def _generate_html_report(self, issues: List[ValidationIssue]) -> str:
        ...
        html_parts = ['<div class="validation-report">', "<h2>📋 Validation Report</h2>"]
        ...
        html_parts.append(
            f'<div class="summary">Summary: {len(errors)} errors, ' f"{len(warnings)} warnings, {len(info)} info</div>"
        )
            ...
            if items:
                ...
                for issue in items:
                    ...
                    if issue.line_number:
                        html_parts.append(f'<span class="line-number">Line {issue.line_number}</span> ')
