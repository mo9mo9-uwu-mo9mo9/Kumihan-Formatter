...
class AsyncParserMixin(EventDrivenParserMixin):
    ...
    def _parse_sync_wrapper(self, content: Union[str, List[str]], kwargs: Dict[str, Any]) -> Any:
        """同期処理ラッパー（ThreadPoolExecutor用）"""
        return self.parse(content, **kwargs)
        
    async def parse_batch_async(self, 
        ...
        try:
            ...
            if errors:
                for index, error in errors:
                    self.notify_parse_error(error, False,
                                          batch_index=index,
                                          batch_processing=True)
            ...
            processing_time = time.time() - start_time
            self.notify_parse_error(e, True,
                                  processing_time=processing_time,
                                  batch_processing=True)
            raise
...
class AsyncParserOrchestrator:
    ...
    async def parse_async(self, parser_name: str, content: Union[str, List[str]], **kwargs) -> Any:
        """非同期パース要求"""
    ...
    async def parse_with_multiple_parsers(self,
                                        parser_configs: List[Dict[str, Any]],
