...
from .parser_monitoring import (
    ParserMonitor,
    DetailedParserLogger,
    get_parser_monitor,
    get_parser_logger,
)
...
class EventDrivenArchitectureSystem:
    ...
    async def initialize(
        self, async_workers: int = 3, enable_monitoring: bool = True
    ) -> None:
        """システム初期化"""
        try:
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "async_queue_status": (
                self.async_orchestrator.get_queue_status()
                if self._async_enabled
                else None
            ),
    ...
    def _generate_recommendations(
        self, issues: List[str], alerts: List[Any]
    ) -> List[str]:
        """推奨事項生成"""
        recommendations = []
    ...
    def enable_debug_mode(self) -> None:
        """デバッグモード有効化"""
        # より詳細なログ出力
        self.logger_system.set_log_level_filter(
