...
class ParserEventBus:
    ...
    def subscribe(
        self, event_type: ParserEventType, handler: ParserEventHandler
    ) -> None:
        """パーサーイベントハンドラー登録"""
    ...
    def subscribe_async(
        self, event_type: ParserEventType, handler: AsyncParserEventHandler
    ) -> None:
        """非同期パーサーイベントハンドラー登録"""
    ...
    def publish(self, event: ParserEvent) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Parser event published: {event.event_type.value} "
    ...
    async def publish_async(self, event: ParserEvent) -> None:
        ...
        try:
            ...
            processing_time = time.time() - start_time
            logger.debug(
                f"Async parser event published: {event.event_type.value} "
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_events_processed": sum(
                len(times) for times in self._performance_metrics.values()
            ),
        }


# パーサーイベント作成ヘルパー関数
def create_parse_started_event(
    parser_name: str, parser_type: str, content_hash: Optional[str] = None
) -> ParserEvent:
    """解析開始イベント作成"""
    return ParserEvent(
        event_type=ParserEventType.PARSE_STARTED,
        data=ParserEventData(
            parser_name=parser_name, parser_type=parser_type, content_hash=content_hash
        ),
    )


def create_parse_completed_event(
    parser_name: str,
    parser_type: str,
...
def create_parser_error_event(
    parser_name: str, parser_type: str, error: Exception, is_critical: bool = False
) -> ParserEvent:
    """エラーイベント作成"""
    return ParserEvent(
        event_type=(
