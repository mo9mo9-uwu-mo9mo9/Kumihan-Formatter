...
class TOCGenerator:
                ...
                if isinstance(node, Node):
                    if node.is_heading():
                        ...
                        if level:
                            ...
                            headings.append(
                                {"level": level, "id": heading_id, "title": node.get_text_content(), "node": node}
                            )
    ...
    def _build_toc_structure(self, headings: List[Dict[str, Any]]) -> List[TOCEntry]:
        ...
        for heading in headings:
            entry = TOCEntry(
                level=heading["level"], title=heading["title"], heading_id=heading["id"], node=heading["node"]
            )
    ...
    def should_generate_toc(self, nodes: List[Node]) -> bool:
        ...
        has_toc_marker = any(isinstance(node, Node) and node.type == "toc" for node in nodes)
    ...
    def get_toc_statistics(self, entries: List[TOCEntry]) -> Dict[str, Any]:
        ...
        stats = {"total_entries": 0, "levels_used": set(), "max_depth": 0, "entries_by_level": {}}
        ...
        def check_ids(entry_list: List[TOCEntry]) -> None:
            for entry in entry_list:
                if entry.heading_id in seen_ids:
                    self.issues.append(f"重複する見出しID: '{entry.heading_id}' " f"('{entry.get_text_content()}')")
                else:
