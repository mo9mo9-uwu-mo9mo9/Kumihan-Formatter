...
class ScenarioDataGenerators:
    ...
    def generate_markup_content(
        ...
        for i in range(size):
            # 基本テキスト生成
            base_text = ''.join(random.choices(string.ascii_letters + string.digits + ' ', k=random.randint(20, 80)))
            ...
            if random.random() < markup_density:
                ...
                if "|||" in pattern:  # ルビ記法
                    ...
                    reading = ''.join(random.choices(string.ascii_lowercase, k=random.randint(3, 8)))
    ...
    @staticmethod
    def generate_nested_structure(depth: int = 5, breadth: int = 3, **kwargs: Any) -> Dict[str, Any]:
        """ネストした構造体を生成
    ...
    @staticmethod
    def generate_test_files(count: int = 10, base_dir: Optional[Path] = None, **kwargs: Any) -> List[Path]:
        """テスト用ファイルを生成
