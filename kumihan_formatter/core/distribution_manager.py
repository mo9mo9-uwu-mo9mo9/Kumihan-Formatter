...
class DistributionManager:
    ...
    def create_user_friendly_distribution(
        self, source_dir: Path, output_dir: Path, convert_docs: bool = True, include_developer_docs: bool = False
    ) -> Dict[str, int]:
        """ユーザーフレンドリーな配布構造を作成
        ...
        Returns:
            Dict: 処理統計（変換ファイル数等）
        """
        stats = {"total_files": 0, "converted_to_html": 0, "converted_to_txt": 0, "copied_as_is": 0, "excluded": 0}
        ...
        if convert_docs:
            stats.update(
                self._process_document_conversion(classified_files, source_dir, output_dir, include_developer_docs)
            )
    ...
    def _copy_program_files(
        self, classified_files: Dict[DocumentType, List[Path]], source_dir: Path, output_dir: Path
    ) -> Dict[str, int]:
        """メインプログラムファイルの処理"""
    ...
    def _create_documentation_index(self, output_dir: Path, classified_files: Dict[DocumentType, List[Path]]) -> None:
        """文書インデックスページを作成"""
...
def create_user_distribution(
    source_dir: Path, output_dir: Path, convert_docs: bool = True, include_developer_docs: bool = False, ui=None
) -> Dict[str, int]:
    """ユーザー向け配布物を作成（外部API）
    ...
    return manager.create_user_friendly_distribution(source_dir, output_dir, convert_docs, include_developer_docs)
