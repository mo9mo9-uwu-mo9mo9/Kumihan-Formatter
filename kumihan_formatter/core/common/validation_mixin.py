    ...
    def validate(self, value: Any) -> Optional[ValidationError]:
        ...
        try:
            ...
            return ValidationError(
                f"Validation rule '{self.name}' failed: {e}", severity=ErrorSeverity.CRITICAL, technical_details=str(e)
            )
...
class ValidationMixin:
    ...
    def require_not_empty(self, field: str, message: Optional[str] = None) -> None:
        ...
        self.add_simple_rule(
            field, lambda x: x is not None and str(x).strip() != "", msg, suggestion=f"Provide a value for {field}"
        )

    def require_type(self, field: str, expected_type: type, message: Optional[str] = None) -> None:
    ...
    def require_matches_pattern(self, field: str, pattern: str, message: Optional[str] = None) -> None:
    ...
    def require_one_of(self, field: str, valid_values: List[Any], message: Optional[str] = None) -> None:
        ...
        self.add_simple_rule(
            field, lambda x: x in valid_values, msg, suggestion=f"Use one of: {', '.join(map(str, valid_values))}"
        )
