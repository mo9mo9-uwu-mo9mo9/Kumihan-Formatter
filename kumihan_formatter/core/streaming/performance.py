    ...
    def stop_profiling(self) -> PerformanceMetrics:
        ...
        if self.metrics.total_time > 0:
            self.metrics.throughput_mbps = (
                self._total_bytes_processed / 1024 / 1024 / self.metrics.total_time
            )
...
class AdaptiveChunkSizer:
    ...
    def get_optimal_chunk_size(
        ...
        # メモリ制約を考慮
        memory_limited_size = min(
            base_size,
            available_memory // 4  # 利用可能メモリの1/4を上限
        )
        
        # パフォーマンス履歴を考慮
        if recent_performance and self._should_adjust_size(recent_performance):
            adjusted_size = self._adjust_based_on_performance(
                memory_limited_size,
                recent_performance
            )
        else:
    ...
    def _should_adjust_size(self, performance: PerformanceMetrics) -> bool:
        ...
        # 処理時間のばらつきが大きい場合
        if (performance.chunk_processing_times and 
            len(performance.chunk_processing_times) > 3):
...
class CacheOptimizer:
    ...
    def optimize_memory_layout(
        self, 
        chunk_access_patterns: List[Tuple[int, float]]
    ) -> Dict[str, Any]:
        """メモリレイアウトを最適化"""
