...
class ProgressInfo:
    ...
    def estimated_remaining_time(self) -> Optional[float]:
        """推定残り時間（秒）"""
        if self.total_bytes is None or self.processing_speed is None or self.processing_speed == 0:
            return None
...
class ProgressTracker:
    ...
    def __init__(self, total_bytes: Optional[int] = None, total_chunks: Optional[int] = None):
        self._info = ProgressInfo(total_bytes=total_bytes, total_chunks=total_chunks)
    ...
    def update(
        self, processed_bytes: Optional[int] = None, current_chunk: Optional[int] = None, message: Optional[str] = None
    ) -> None:
        """進捗を更新"""
        with self._lock:
    ...
    def increment(self, bytes_processed: int = 0, chunks_processed: int = 0, message: Optional[str] = None) -> None:
        """進捗を増分更新"""
        with self._lock:
            if self._info.state != ProgressState.RUNNING:
                return
...
class ConsoleProgressTracker(ProgressTracker):
    ...
    def __init__(
        self, total_bytes: Optional[int] = None, total_chunks: Optional[int] = None, update_interval: float = 1.0
    ):
    ...
    def _console_callback(self, info: ProgressInfo) -> None:
        ...
        # 表示間隔チェック
        if current_time - self._last_display_time < self.update_interval and info.state == ProgressState.RUNNING:
            return
