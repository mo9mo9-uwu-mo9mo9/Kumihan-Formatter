...
class ChunkManager:
    ...
    def _create_chunks_from_stream(
        self,
        stream: io.TextIOBase,
        chunk_size: int
    ) -> Iterator[Tuple[str, ChunkMetadata]]:
        ...
        while True:
            ...
            # ブロック境界でチャンクを分割
            chunk_content, remaining_buffer = self._split_at_block_boundary(
                buffer, 
                chunk_size
            )
    ...
    def merge_incomplete_blocks(
        ...
        previous_chunk: str,
        current_chunk: str,
        previous_metadata: ChunkMetadata,
        current_metadata: ChunkMetadata
    ) -> Tuple[str, str]:
        """不完全なブロックをマージ"""
        
        # 前のチャンクに不完全な終了がある場合
        if (previous_metadata.incomplete_block_start is not None and
            current_metadata.incomplete_block_end is not None):
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for i, (content, metadata) in enumerate(
                    self._create_chunks_from_stream(f, chunk_size)
                ):
                    if i == chunk_index:
                        # プレビュー用に先頭部分のみ返す
