...
class ContainerConfigurator:
    ...
    def configure_from_file(self, config_path: Path, environment: Optional[str] = None) -> None:
        """ファイルから設定を適用"""
        config = self.loader.load_from_file(config_path)
        self.apply_configuration(config, environment)

    def configure_from_dict(self, config_data: Dict[str, Any], environment: Optional[str] = None) -> None:
        """辞書から設定を適用"""
        config = self.loader.load_from_dict(config_data)
        self.apply_configuration(config, environment)

    def apply_configuration(self, config: DIConfiguration, environment: Optional[str] = None) -> None:
        """設定をコンテナに適用"""

        # 環境固有のサービスをフィルター
        filtered_services = self._filter_services_by_environment(config.services, environment)
    ...
    def _filter_services_by_environment(
        self, services: List[ServiceConfiguration], environment: Optional[str]
    ) -> List[ServiceConfiguration]:
        """環境に基づいてサービスをフィルター"""
        if environment is None:
            return [s for s in services if s.environment is None]

        return [s for s in services if s.environment is None or s.environment == environment]
    ...
    def _create_factory(self, factory_method: str, parameters: Dict[str, Any]) -> callable:
        """ファクトリーメソッドを作成"""
    ...
    def _discover_services_in_module(self, module) -> None:
        ...
        for attr_name in dir(module):
            ...
            if hasattr(attr, "__class__") and hasattr(attr, "_auto_register") and attr._auto_register:
