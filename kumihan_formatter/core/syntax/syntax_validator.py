...
class KumihanSyntaxValidator:
    ...
    def get_friendly_errors(self) -> List[UserFriendlyError]:
        ...
        for error in self.errors:
            if error.error_type == ErrorTypes.ENCODING:
            ...
            elif error.error_type in [ErrorTypes.INVALID_KEYWORD, ErrorTypes.UNKNOWN_KEYWORD]:
                ...
                friendly_error = ErrorCatalog.create_syntax_error(
                    line_num=error.line_number, invalid_content=invalid_content, file_path=self.current_file
                )
            else:
                # Other errors as general syntax errors
                friendly_error = ErrorCatalog.create_syntax_error(
                    line_num=error.line_number, invalid_content=error.message, file_path=self.current_file
                )
    ...
    def validate_file(self, file_path: Path) -> List[SyntaxError]:
        ...
        try:
            ...
            self._add_error(
                1, 1, ErrorSeverity.ERROR, ErrorTypes.FILE_NOT_FOUND, "ファイルが見つかりません", str(file_path)
            )
            return self.errors
    ...
    def _check_multiline_syntax(
        self, line_num: int, stripped: str, block_start_line: int, existing_keywords: List[str]
    ) -> None:
    ...
    def _validate_single_keyword(self, line_num: int, keyword: str, context: str) -> None:
    ...
    def _validate_keyword_combination(self, line_num: int, keywords: List[str], context: str) -> None:
