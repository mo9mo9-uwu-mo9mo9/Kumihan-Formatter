...
class BlockParser:
    ...
    def parse_block_marker(self, lines: List[str], start_index: int) -> Tuple[Optional[Node], int]:
        ...
        if marker_content.startswith("画像") or ";;;画像" in opening_line or self._is_simple_image_marker(opening_line):
            return self._parse_image_block(lines, start_index)

        # Find closing marker
        is_valid, end_index, validation_errors = self.marker_validator.validate_block_structure(lines, start_index)
    ...
    def is_opening_marker(self, line: str) -> bool:
        ...
        return line.startswith(";;;") and line != ";;;" and not (line.endswith(";;;") and line.count(";;;") > 1)
