...
class ListParser:
    ...
    def _parse_keyword_list_item(self, content: str) -> Node:
        ...
        if len(keywords) == 1:
            # Single keyword
            styled_content = self.keyword_parser.create_single_block(keywords[0], text_content, attributes)
        else:
            # Compound keywords
            styled_content = self.keyword_parser.create_compound_block(keywords, text_content, attributes)
