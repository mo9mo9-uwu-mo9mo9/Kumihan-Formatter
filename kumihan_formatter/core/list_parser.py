...
class ListParser:
    ...
    def parse(
        ...
        try:
            ...
            # ノード作成
            nodes = self._create_nodes_from_parsed_data(parsed_data)
            ...
            if hasattr(ParseResult, "__dataclass_fields__"):
                return ParseResult(
                    ...
                    metadata={
                        "parser": "ListParser",
                        "list_depth": self._calculate_list_depth(parsed_data),
                        "total_items": self._count_total_items(parsed_data),
                    },
                )
            else:
                # フォールバック: 辞書で返却
    ...
    def parse_list_items(
        ...
        try:
            parsed_data = parse_list_string(content)
            return self._create_nodes_from_parsed_data(parsed_data, create_items=True)
        except Exception:
            # エラー時は空リスト返却
            return []
    ...
    def parse_nested_list(
        ...
        try:
            parsed_data = parse_list_string(content)
            return self._create_nested_nodes(parsed_data, level)
        except Exception:
            return []
    ...
    def _create_nodes_from_parsed_data(
        self, data: Any, create_items: bool = False
    ) -> List[Node]:
        """パースデータからノード作成
        ...
        if isinstance(data, list):
            for i, item in enumerate(data):
                if isinstance(item, list):
                    # ネストリスト
                    child_nodes = self._create_nodes_from_parsed_data(
                        item, create_items
                    )
                    node = Node(
                        type="nested_list",
    ...
    def _create_nested_nodes(self, data: Any, level: int) -> List[Node]:
        """ネストノード作成（レベル指定）
        ...
        if isinstance(data, list):
            for i, item in enumerate(data):
                if isinstance(item, list):
                    child_nodes = self._create_nested_nodes(item, level + 1)
                    node = Node(
                        type="nested_list",
                        content=child_nodes,
    ...
    def _calculate_list_depth(self, data: Any, current_depth: int = 0) -> int:
        """リスト深度計算
    ...
    def _count_total_items(self, data: Any) -> int:
        """総アイテム数カウント
        ...
        for item in data:
            if isinstance(item, list):
                count += self._count_total_items(item)
            else:
                count += 1
