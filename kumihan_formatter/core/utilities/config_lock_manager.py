...
class ConfigLockManager:
    ...
    def __init__(self):
        ...
        self._cleanup_thread = threading.Thread(
            target=self._cleanup_expired_locks, daemon=True, name="ConfigLockCleanup"
        )
    ...
    def acquire_config_lock(
        ...
        try:
            ...
            # 読み書きロック制御
            self._handle_read_write_lock(
                file_key, mode, timeout - (time.time() - start_time)
            )
            ...
            if acquired:
                try:
                    ...
                    logger.debug(
                        f"ロック解除完了: {file_key} ({mode.value}) by {thread_id}"
                    )

                except Exception as e:
                    logger.error(f"ロック解除エラー: {file_key} - {e}")

    def _handle_read_write_lock(
        self, file_key: str, mode: LockMode, remaining_timeout: float
    ):
        """読み書きロック制御"""
        if mode == LockMode.READ:
            # 読み取りロック: 他の読み取りと共存可能
            with self._active_locks_lock:
                self._read_lock_counts[file_key] = (
                    self._read_lock_counts.get(file_key, 0) + 1
                )
            ...
            while True:
                with self._active_locks_lock:
                    ...
                    active_locks = [
                        ...
                        if str(lock.file_path) == file_key
                        and lock.owner_thread != threading.current_thread().name
                    ]

                    if (
                        not active_locks
                        and self._read_lock_counts.get(file_key, 0) == 0
                    ):
                        break
        ...
        if mode == LockMode.READ:
            with self._active_locks_lock:
                if file_key in self._read_lock_counts:
                    self._read_lock_counts[file_key] = max(
                        0, self._read_lock_counts[file_key] - 1
                    )
    ...
    def read_config_safe(
        ...
        with self.acquire_config_lock(
            file_path, LockMode.READ, timeout, f"読み取り: {file_path.name}"
        ):
            try:
    ...
    def write_config_safe(
        ...
        with self.acquire_config_lock(
            file_path, LockMode.WRITE, timeout, f"書き込み: {file_path.name}"
        ):
            try:
                # ディレクトリ作成
                ...
                status["lock_details"].append(
                    {
                        ...
                        "expires_at": (
                            lock_info.expires_at.isoformat()
            ...
            try:
                ...
                with self._active_locks_lock:
                    for lock_key, lock_info in list(self._active_locks.items()):
                        if lock_info.expires_at and now > lock_info.expires_at:
                    ...
                    for lock_key in expired_locks:
                        ...
                        # 読み取りロックカウンターも調整
                        if (
                            lock_info.mode == LockMode.READ
                            and file_key in self._read_lock_counts
                        ):
                            self._read_lock_counts[file_key] = max(
                                0, self._read_lock_counts[file_key] - 1
                            )
...
def safe_read_config(
    file_path: Union[str, Path], default: Any = None, timeout: Optional[float] = None
) -> Any:
    """安全な設定ファイル読み取り（関数版）"""
...
def safe_write_config(
    ...
    """安全な設定ファイル書き込み（関数版）"""
    return get_config_lock_manager().write_config_safe(
        file_path, data, timeout, backup, atomic
    )
...
def config_lock(
    ...
    with get_config_lock_manager().acquire_config_lock(
        file_path, mode, timeout, operation_description
    ):
        yield
