...
# 統合システムからインポート
try:
    from kumihan_formatter.core.utilities.ast_performance_optimizer import parse_file_fast, get_ast_optimizer
...
class UnifiedSecurityScanner:
    ...
    def __init__(self, project_root: Path, config_path: Optional[Path] = None):
        ...
        self.config_path = config_path or self.project_root / "config" / "unified_scanner.json"
        self.config = self._load_config()
        
        # メモリ効率的な検出結果管理
        max_findings = self.config.get("memory_management", {}).get("max_findings", 1000)
    ...
    def scan_files_for_vulnerabilities(
        ...
        custom_analyzer: Optional[Callable[[ast.AST, Path, str], List[SecurityFinding]]] = None
    ) -> ScanResult:
        """統一ファイルスキャン実行"""
        ...
        try:
            ...
            if USE_INTEGRATED_SYSTEMS:
                ...
                # 並列処理対応
                if self.config.get("performance", {}).get("enable_parallel_scanning", True):
                    ast_results = optimizer.parse_files_parallel(target_files)
                else:
                    ast_results = {f: optimizer.parse_file_optimized(f) for f in target_files}
            else:
                # フォールバック: 従来のAST解析
                ast_results = {}
                ...
                try:
                    ...
                    if custom_analyzer:
                        ...
                        findings = self._analyze_ast_for_patterns(
                            ast_tree, file_path, content, 
                            scanner_type, vulnerability_patterns
                        )
                        for finding in findings:
                            self.findings_list.add_finding(finding)
                    ...
                    # 定期的なガベージコレクション
                    if files_scanned % self.config.get("memory_management", {}).get("gc_interval", 100) == 0:
                        gc.collect()
                    ...
                    if USE_INTEGRATED_SYSTEMS:
                        ...
                        sanitized_error = safe_handle_exception(
                            e,
                            context={"file_path": str(file_path), "scanner_type": scanner_type.value},
                            user_exposure=ExposureRisk.INTERNAL,
                            severity=ErrorSeverity.LOW
                        )
                        logger.debug(f"ファイルスキャンエラー [{sanitized_error.trace_id}]: {sanitized_error.user_message}")
                    else:
            ...
            logger.info(f"統一スキャン完了 ({scanner_type.value}): {files_scanned}ファイル, {len(result.findings)}件検出")
            return result
            ...
            if USE_INTEGRATED_SYSTEMS:
                sanitized_error = safe_handle_exception(
                    e,
                    context={"scanner_type": scanner_type.value, "project_root": str(self.project_root)},
                    user_exposure=ExposureRisk.INTERNAL,
                    severity=ErrorSeverity.HIGH
                )
                logger.error(f"統一スキャンエラー [{sanitized_error.trace_id}]: {sanitized_error.user_message}")
                raise Exception(f"スキャン実行失敗 [{sanitized_error.error_code}]: {sanitized_error.user_message}")
            else:
...
def scan_for_platform_issues(project_root: Path, patterns: Dict[str, str]) -> ScanResult:
    """プラットフォーム互換性スキャン（関数版）"""
    scanner = get_unified_scanner(project_root)
