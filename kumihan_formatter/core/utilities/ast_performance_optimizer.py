...
class ASTPerformanceOptimizer:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        """初期化"""
        self.config_path = config_path or Path(__file__).parent.parent.parent.parent / "config" / "ast_optimization.json"
        self.config = self._load_config()
        
        # キャッシュ設定
        self.cache_level = CacheLevel(self.config.get("cache_level", "hybrid"))
        self.max_memory_cache_size = self.config.get("max_memory_cache_size", 100)  # エントリ数
        ...
        # ディスクキャッシュ
        self.disk_cache_dir = Path(self.config.get("disk_cache_dir", "/tmp/kumihan_ast_cache"))
        self.disk_cache_dir.mkdir(parents=True, exist_ok=True)
        
        # パフォーマンス追跡
        self.metrics = ParseMetrics(
            total_files=0, cached_hits=0, cache_misses=0,
            total_parse_time=0.0, peak_memory_usage=0,
            gc_collections=0, memory_leaks=[]
        )
        ...
        try:
            if strategy == ParseStrategy.FULL:
                ...
                try:
                    ...
                    # ノード数制限
                    max_nodes = self.config.get("memory_optimization", {}).get("max_ast_nodes", 10000)
    ...
    def _selective_parse(self, content: str) -> Optional[ast.AST]:
        ...
        try:
            ...
            for node in tree.body:
                ...
                if isinstance(node, ast.FunctionDef) and selective_config.get("parse_function_defs", True):
                    include_node = True
                elif isinstance(node, ast.ClassDef) and selective_config.get("parse_class_defs", True):
                    include_node = True
                elif isinstance(node, (ast.Import, ast.ImportFrom)) and selective_config.get("parse_imports", True):
                    include_node = True
    ...
    def parse_file_optimized(
        ...
        try:
            ...
            if parsed_ast is not None:
                self._cache_ast(cache_key, file_path, content_hash, parsed_ast, 
                               parse_time, memory_used, strategy)
                
                # 弱参照でメモリリーク監視
                self._track_ast_reference(parsed_ast)
            
            # 定期的なGC実行
            if self.metrics.total_files % self.config.get("memory_optimization", {}).get("force_gc_interval", 50) == 0:
    ...
    def _force_garbage_collection(self):
        ...
        if alive_refs > 100:  # 閾値
            ...
            self.metrics.memory_leaks.append(f"AST references: {alive_refs} at {datetime.now()}")
        ...
        while True:
            try:
                ...
                with self._cache_lock:
                    ...
                    if expired_keys:
                        logger.debug(f"期限切れキャッシュ削除: {len(expired_keys)}エントリ")
    ...
    def _cleanup_disk_cache(self):
        ...
        try:
            ...
            if cleaned_files > 0:
                logger.debug(f"ディスクキャッシュクリーンアップ: {cleaned_files}ファイル削除")
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        with self._cache_lock:
            ...
            return {
                ...
                "active_ast_references": len([ref for ref in self._ast_references if ref() is not None]),
                "memory_leaks_detected": len(self.metrics.memory_leaks),
                "disk_cache_files": len(list(self.disk_cache_dir.glob("*.pkl"))) if self.disk_cache_dir.exists() else 0
            }
