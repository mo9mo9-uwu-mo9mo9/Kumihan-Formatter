...
import threading
from typing import Iterator, List, Callable, Optional, Dict, Any
...
class ParallelChunkProcessor:
    ...
    def __init__(self, max_workers: Optional[int] = None, chunk_size: int = 100):
        ...
        self.logger.info(f"ParallelChunkProcessor initialized: {self.max_workers} workers, chunk_size={chunk_size}")
    ...
    def _process_single_chunk(
        ...
        try:
            with self._lock:
                self.logger.debug(f"Processing chunk {chunk.chunk_id}: lines {chunk.start_line}-{chunk.end_line}")
    ...
    def create_chunks_from_lines(self, lines: List[str], chunk_size: Optional[int] = None) -> List[ChunkInfo]:
        """行リストからチャンクを作成"""
    ...
    def create_chunks_from_file(self, file_path: Path, chunk_size: Optional[int] = None) -> List[ChunkInfo]:
...
class ParallelStreamingParser:
    ...
    def __init__(self, max_workers: Optional[int] = None, chunk_size: int = 100):
        ...
        self.logger.info(f"Thread-safe parallel streaming parser initialized with {max_workers} workers")
    ...
    def parse_text_parallel(
        ...
        Yields:
            Any: 解析済みノード
        """
        self.logger.info(f"Starting thread-safe parallel streaming parse of text: {len(text)} characters")
        ...
        try:
            ...
            while current < len(chunk.lines) and not self._shutdown_requested:
                try:
                    ...
                    elif line.startswith('#') and not parser_components['block_parser'].is_opening_marker(line):
                        current += 1  # コメント行スキップ
                    ...
                    self.logger.warning(f"Error processing line {chunk.start_line + current} in chunk {chunk.chunk_id}: {e}")
                    current += 1  # エラー時は次の行へ
    ...
    def _get_thread_local_parser_components(self) -> Dict[str, Any]:
        ...
        if not hasattr(self._thread_local, 'parser_components'):
            ...
            self.logger.debug(f"Created thread-local parser components for thread {threading.get_ident()}")
    ...
    def request_shutdown(self):
        ...
        if self._active_threads:
            self.logger.warning(f"Timeout waiting for {len(self._active_threads)} threads to finish")
        else:
