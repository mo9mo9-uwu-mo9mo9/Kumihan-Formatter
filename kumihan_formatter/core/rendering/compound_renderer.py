...
from .html_utils import sort_keywords_by_nesting_order, process_text_content
...
class CompoundElementRenderer:
    ...
    def render_compound_element(self, keywords: List[str], content: str, 
    ...
    def _wrap_with_keyword(self, content: str, keyword: str, 
                          attributes: Dict[str, Any]) -> str:
        """
    ...
    def render_keyword_list_item(self, keywords: List[str], content: str, 
                               attributes: Dict[str, Any]) -> str:
        """
    ...
    def validate_keyword_combination(self, keywords: List[str]) -> tuple[bool, str]:
        ...
        if len(heading_keywords) > 1:
            return False, f"Multiple heading levels not allowed: {', '.join(heading_keywords)}"
        
        # Check for duplicate details types
        details_keywords = [k for k in keywords if k in ['折りたたみ', 'ネタバレ']]
        if len(details_keywords) > 1:
            return False, f"Multiple details types not allowed: {', '.join(details_keywords)}"
        
        # Check for unknown keywords
        known_keywords = {
            '太字', 'イタリック', '枠線', 'ハイライト', '折りたたみ', 'ネタバレ',
