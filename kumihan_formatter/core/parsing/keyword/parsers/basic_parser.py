...
class BasicKeywordParser:
    ...
    def parse_keyword_string(self, keyword_text: str) -> Dict[str, Any]:
        """キーワード文字列の解析

        Args:
            keyword_text: 解析対象のキーワード文字列
        ...
        if attr_match:
            ...
            result["attributes"] = self.parse_attributes(attr_text)
    ...
    def create_keyword_node(self, keyword_info: Dict[str, Any]) -> Node:
        """キーワード情報からノードを作成

        Args:
            keyword_info: キーワード解析情報
        ...
        if definition:
            ...
            node.metadata.update(
                {
                    ...
                    "context": keyword_info.get("context", "unknown"),
                    "position": keyword_info.get("position", (0, 0)),
                }
            )
            ...
            # 属性ハンドラーの適用
            self.apply_attribute_handlers(node, keyword_info["attributes"])
    ...
    def apply_attribute_handlers(self, node: Node, attributes: Dict[str, Any]) -> None:
        """属性ハンドラーを適用
    ...
    def is_valid_keyword(self, keyword: str) -> bool:
        ...
        return keyword in self.default_keywords or keyword in self.custom_keywords
