...
Issue #912対応: 2つのConfigValidatorを統合
- core/config/config_validator.py (基本スキーマ検証)
- core/unified_config/config_validator.py (Pydantic+Kumihan特有検証)
"""
...
class EnhancedValidationResult:
    ...
    def to_validation_result(self) -> ValidationResult:
        """基本のValidationResultに変換"""
        return ValidationResult(
            is_valid=self.is_valid, errors=self.errors, warnings=self.warnings
        )
...
class ConfigValidator:
    ...
    基本スキーマ検証とPydantic+Kumihan特有検証を統合
    """

    # 必須設定スキーマ（従来のconfig_validatorから継承）
    SCHEMA = {
        "markers": {
            "type": dict,
            "required": True,
    ...
    def validate(self, config: dict[str, Any]) -> ValidationResult:
        """基本設定をスキーマに対して検証（従来のvalidateメソッド）"""
        errors: list[str] = []
        warnings: list[str] = []
    ...
    def validate_config(
        ...
        try:
            ...
            # 2. 従来のスキーマ検証も実行
            basic_result = self.validate(config_data)
            ...
            self.logger.error(f"Pydantic検証エラー: {e}")
    ...
    def _validate_section(
        ...
        """設定セクションを検証（従来の_validate_sectionメソッド）"""
        errors: list[str] = []
        warnings: list[str] = []
    ...
    def _validate_parallel_config(
        self, config: ParallelConfig, result: EnhancedValidationResult
    ) -> None:
        """並列処理設定の検証"""
    ...
    def _validate_logging_config(
        self, config: LoggingConfig, result: EnhancedValidationResult
    ) -> None:
        """ログ設定の検証"""
    ...
    def _validate_error_config(
        self, config: ErrorConfig, result: EnhancedValidationResult
    ) -> None:
        """エラー設定の検証"""
    ...
    def _validate_rendering_config(
        self, config: Any, result: EnhancedValidationResult  # RenderingConfig
    ) -> None:
        """レンダリング設定の検証"""
    ...
    def _validate_ui_config(
        self, config: Any, result: EnhancedValidationResult
    ) -> None:  # UIConfig
        """UI設定の検証"""
    ...
    def _validate_config_dependencies(
        self, config: KumihanConfig, result: EnhancedValidationResult
    ) -> None:
        """設定間の依存関係検証"""
    ...
    def _apply_auto_fixes(
        ...
        """
        fixed_config = config.model_copy(deep=True)
