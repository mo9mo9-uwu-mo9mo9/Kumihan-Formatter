...
class ErrorConfigManager:
    ...
    # サポートする設定ファイル名
    CONFIG_FILES = ["kumihan.toml", "kumihan.yaml", "kumihan.yml", ".kumihan.toml", ".kumihan.yaml"]
    ...
    def _load_configuration(self) -> None:
        ...
        self.logger.info(f"Error handling configuration loaded: level={self.config.default_level.value}")
    ...
    def _load_from_environment(self) -> None:
        ...
        if suggestions_str := os.getenv(f"{self.ENV_PREFIX}SUGGESTIONS"):
            self.config.show_suggestions = suggestions_str.lower() in ['true', '1', 'yes']
