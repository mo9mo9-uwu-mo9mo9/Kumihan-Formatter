        ...
        if self.fix_suggestions:
            ...
            for i, suggestion in enumerate(self.fix_suggestions, 1):
                confidence_emoji = (
                    ...
                    else "💭" if suggestion.confidence >= 0.7 else "🤔"
                )
        ...
        if error.fix_suggestions:
            ...
            for i, suggestion in enumerate(error.fix_suggestions, 1):
                confidence_text = (
                    "高"
                    if suggestion.confidence >= 0.9
                    else "中" if suggestion.confidence >= 0.7 else "低"
                )
                lines.append(
                    f"  {i}. {suggestion.description} (信頼度: {confidence_text})"
                )
...
class ErrorReportBuilder:
    ...
    def create_keyword_error(
        ...
        # 類似キーワードの提案
        similar_keywords = ErrorReportBuilder._find_similar_keywords(
            invalid_keyword, valid_keywords
        )
        if similar_keywords:
    ...
    def _get_context_lines(
        file_path: Path, line_number: int, context_count: int = 3
    ) -> List[str]:
        """指定した行の周辺コンテキストを取得"""
        try:
    ...
    def _generate_smart_suggestions(
        error_type: str, problem_text: str
    ) -> List[FixSuggestion]:
        """エラータイプ別のスマート修正提案を生成"""
        suggestions = []
