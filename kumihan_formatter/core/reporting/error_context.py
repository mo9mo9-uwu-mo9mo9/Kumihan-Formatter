...
class ErrorContextManager:
    ...
    def get_highlighted_line(self, line_number: int) -> Optional[str]:
        """指定行をハイライト表示用に取得"""
        if not self._file_lines or line_number <= 0 or line_number > len(self._file_lines):
            return None
        
        return self._file_lines[line_number - 1].rstrip('\n')
    
    def extract_problem_section(self, line_number: int, column: Optional[int] = None) -> str:
        """問題箇所を視覚的に強調表示"""
