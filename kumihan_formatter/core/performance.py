...
class PerformanceReport:
    ...
    def __str__(self) -> str:
        ...
        if self.throughput_nodes_per_second:
            lines.append(f"Throughput: {self.throughput_nodes_per_second:.1f} nodes/sec")
...
class PerformanceMonitor:
    ...
    @contextmanager
    def measure(self, operation_name: str, node_count: Optional[int] = None, 
                file_size: Optional[int] = None):
    ...
    def _check_performance_warnings(self, report: PerformanceReport) -> None:
        ...
        if report.memory_usage and report.memory_usage > 100 * 1024 * 1024:  # 100MB
            report.add_warning(f"High memory usage: {report._format_bytes(report.memory_usage)}")
        
        # Low cache hit ratio warning
        if report.cache_hit_ratio < 0.5 and (report.cache_hits + report.cache_misses) > 10:
            report.add_warning(f"Low cache hit ratio: {report.cache_hit_ratio:.1%}")
        
        # Low throughput warning for large files
        if (report.file_size and report.file_size > 1024 * 1024 and  # > 1MB
            report.throughput_mb_per_second and report.throughput_mb_per_second < 1.0):
            report.add_warning(f"Low file throughput: {report.throughput_mb_per_second:.2f} MB/s")
...
class PerformanceProfiler:
    ...
    def get_top_functions(self, metric: str = "total_time", limit: int = 10) -> List[tuple]:
...
class PerformanceOptimizer:
    ...
    @staticmethod
    def should_use_streaming(file_size: int, available_memory: Optional[int] = None) -> bool:
        ...
        if available_memory and file_size > available_memory * 0.1:  # 10% of available memory
            return True
...
class BenchmarkSuite:
    ...
    def run_benchmark(self, name: str, func: Callable, *args, iterations: int = 5, **kwargs):
    ...
    def _print_benchmark_summary(self, name: str, results: List[PerformanceReport]):
        ...
        print(f"Std Dev: {statistics.stdev(execution_times) if len(execution_times) > 1 else 0:.3f}s")
        
        if results[0].memory_usage:
            memory_usages = [r.memory_usage for r in results if r.memory_usage]
            if memory_usages:
                avg_memory = statistics.mean(memory_usages)
                print(f"Average Memory: {PerformanceReport._format_bytes(int(avg_memory))}")
    
    def compare_benchmarks(self, benchmark1: str, benchmark2: str) -> Dict[str, float]:
        """Compare two benchmarks"""
        if benchmark1 not in self.benchmark_results or benchmark2 not in self.benchmark_results:
