...
class UnifiedMainParser(BaseParserProtocol):
    ...
    def __init__(self, graceful_errors: bool = False):
        ...
        try:
            ...
            self.logger.warning(
                f"Core parser initialization failed: {e}, using legacy parser"
            )
            self.legacy_parser = LegacyParser(graceful_errors=graceful_errors)
    ...
    def _create_error_parse_result(
        self, error_msg: str, original_text: str
    ) -> "ParseResult":
        """エラー結果作成"""
        from ..core.parsing.base.parser_protocols import ParseResult
        ...
        return ParseResult(
            ...
            errors=[error_msg],
            warnings=[],
            metadata={"original_content": original_text, "graceful_error": True},
        )
        ...
        try:
            if self.core_parser and hasattr(self.core_parser, "validate_syntax"):
                ...
                return (
                    result
                    if isinstance(result, dict)
                    else {"valid": False, "errors": ["Invalid result type"]}
                )
            else:
                # フォールバック: 解析試行で検証
                try:
