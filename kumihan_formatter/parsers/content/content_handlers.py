...
class ContentHandler:
    ...
    def analyze_entities(self, content: str, patterns: Dict[str, re.Pattern[str]]) -> Dict[str, Any]:
        """エンティティの解析"""
        ...
        return {
            ...
            "entity_density": sum(len(ent_list) for ent_list in entities.values()) / len(content.split()) if content.split() else 0,
        }
    
    def analyze_semantic_structure(self, content: str, patterns: Dict[str, re.Pattern[str]]) -> Dict[str, Any]:
        """セマンティック構造の解析"""
        semantic_data = {}
...
class ContentAnalyzer:
    ...
    def parse_mixed_content(self, content: str) -> Dict[str, Any]:
        ...
        mixed_analysis["statistics"] = {
            "kumihan_ratio": len(mixed_analysis["kumihan_blocks"]) / len(lines) if lines else 0,
            "markdown_ratio": len(mixed_analysis["markdown_elements"]) / len(lines) if lines else 0,
            "plain_ratio": len(mixed_analysis["plain_text"]) / len(lines) if lines else 0,
        }
    ...
    def extract_text_summary(self, parse_result: Any) -> Dict[str, Any]:
        ...
        return {
            "summary": sentences[0].strip() if sentences else "サマリーを生成できませんでした",
    ...
    def suggest_improvements(self, parse_result: Any) -> List[str]:
        ...
        if not hasattr(parse_result, 'nodes') or not parse_result.nodes:
            suggestions.append("コンテンツが解析されていません。有効なコンテンツを提供してください。")
            return suggestions
        ...
        if node_count == 0:
            ...
            suggestions.append("コンテンツが非常に長いです。セクションに分割することを検討してください。")
        ...
        if text_content:
            ...
            # 可読性の提案
            avg_sentence_length = len(full_text.split()) / max(1, full_text.count('.') + full_text.count('。'))
            if avg_sentence_length > 25:
                suggestions.append("文章が長すぎる可能性があります。短く分割することを検討してください。")
            
            # 構造の提案
            if full_text.count('#') == 0 and len(full_text) > 500:
                suggestions.append("長いコンテンツには見出しを追加することをお勧めします。")
