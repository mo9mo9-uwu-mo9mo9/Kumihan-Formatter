...
class UnifiedContentParser(BaseParserProtocol):
    ...
    def parse(self, content: str, context: Optional["ParseContext"] = None) -> "ParseResult":
        ...
        try:
            ...
            structure_analysis = self.content_handler.analyze_content_structure(normalized_content)
            entity_analysis = self.content_handler.analyze_entities(normalized_content, self.content_patterns)
            semantic_analysis = self.content_handler.analyze_semantic_structure(normalized_content, self.content_patterns)
    ...
    def extract_footnotes_from_text(self, content: str) -> tuple[str, List[Dict[str, str]]]:
        """脚注抽出（ユーティリティ関数に委譲）"""
