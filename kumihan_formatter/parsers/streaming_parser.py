...
try:
    from ..core.utilities.streaming_parser import (
        ParallelStreamingParser as CoreStreamingParser,
    )
except ImportError:
    pass
...
class UnifiedStreamingParser(StreamingParserProtocol):
    ...
    def __init__(self) -> None:
        ...
        if LegacyStreamingParser is not None:
            try:
                ...
                self.logger.warning(
                    f"Legacy streaming parser initialization failed: {e}"
                )
    ...
    def _create_error_parse_result(
        ...
        return ParseResult(
            ...
            errors=[error_msg],
            warnings=[],
            metadata={"original_content": original_content, "parser_type": "streaming"},
        )
