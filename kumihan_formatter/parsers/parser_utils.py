...
class ParserUtils:
    ...
    @staticmethod
    def split_by_delimiter(text: str, delimiter: str, max_split: Optional[int] = None) -> List[str]:
        """区切り文字による分割"""
    ...
    @classmethod
    def count_nested_levels(cls, text: str, open_char: str = '{', close_char: str = '}') -> int:
        """ネスト深度計算"""
        level = 0
        max_level = 0
        for char in text:
    ...
    @classmethod
    def format_error_message(cls, error: str, line: int = None, column: int = None) -> str:
        """エラーメッセージフォーマット"""
