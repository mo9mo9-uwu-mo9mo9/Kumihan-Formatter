...
def extract_list_block(content: str, start_line: int) -> Tuple[List[str], int]:
    ...
    base_indent = get_list_nesting_level(lines[start_line]) if start_line < len(lines) else 0
...
def validate_regular_lists(content: str) -> List[str]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if list_type:
            ...
            if current_list_type is None:
                ...
                errors.append(f"行 {i}: リストタイプが一貫していません ({current_list_type} vs {list_type})")
    ...
    if list_numbers:
        ...
        for i, number in enumerate(list_numbers):
            if number != expected:
                errors.append(f"順序付きリストの番号が不連続です: 期待値 {expected}, 実際 {number}")
            expected = number + 1
...
def validate_kumihan_list_block(content: str) -> List[str]:
    ...
    for match in matches:
        ...
        if not list_content:
            ...
            errors.extend([f"Kumihanリストブロック内: {error}" for error in inner_errors])
    ...
    while i < len(lines):
        ...
        if line.strip().endswith(":") and not line.startswith(" "):
            ...
            if not has_definition:
                errors.append(f"定義項目 '{line.strip()}' に対応する定義内容がありません")
        else:
            i += 1
