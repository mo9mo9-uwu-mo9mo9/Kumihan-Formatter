...
class Renderer:
    ...
    def render(self, ast: List[Node], config=None, template=None, title=None, 
               source_text=None, source_filename=None, navigation_html=None) -> str:
        """
        Render AST to HTML
        ...
        should_show_toc = (
            toc_data['has_toc'] or 
            any(isinstance(node, Node) and node.type == "toc" for node in ast)
        )
    ...
    def render_with_custom_context(self, ast: List[Node], template_name: str, 
...
def render(ast: List[Node], config=None, template=None, title=None, 
           source_text=None, source_filename=None, navigation_html=None) -> str:
    """
