...
class Renderer:
    ...
    def render(
        ...
        body_ast = [node for node in ast if not (isinstance(node, Node) and node.type == "toc")]
        ...
        should_show_toc = toc_data["has_toc"] or any(isinstance(node, Node) and node.type == "toc" for node in ast)

        # Select template
        template_name = self.template_manager.select_template_name(source_text=source_text, template=template)
    ...
    def render_with_custom_context(self, ast: List[Node], template_name: str, custom_context: Dict[str, Any]) -> str:
        """
        ...
        # Build base context
        context = RenderContext().body_content(body_content).toc_html(toc_data["html"]).has_toc(toc_data["has_toc"])
