...
class ConsoleUI:
    ...
    def encoding_error(self, file_path: str) -> None:
        ...
        self.console.print(f"[red][エラー] エンコーディングエラー:[/red] ファイルを UTF-8 として読み込めません")
    ...
    def unexpected_error(self, error: str) -> None:
        ...
        self.console.print("[dim]   詳細なエラー情報が必要な場合は、GitHubのissueで報告してください[/dim]")
    ...
    def validation_warning(self, error_count: int, is_sample: bool = False) -> None:
        ...
        if is_sample:
            self.console.print(f"[yellow][警告]  {error_count}個のエラーが検出されました（想定されたエラーです）[/yellow]")
            self.console.print("[yellow]   これらのエラーは、記法の学習用にわざと含まれています[/yellow]")
            self.console.print("[yellow]   HTMLファイルでエラー箇所を確認してください[/yellow]")
        else:
            self.console.print(f"[yellow][警告]  警告: {error_count}個のエラーが検出されました[/yellow]")
            self.console.print("[yellow]   HTMLファイルでエラー箇所を確認してください[/yellow]")
    ...
    def files_missing(self, files: list) -> None:
        ...
        self.console.print(f"[red][エラー] {len(files)}個の画像ファイルが見つかりません:[/red]")
        for filename in files:
            self.console.print(f"[red]   - {filename}[/red]")
    
    def duplicate_files(self, duplicates: dict) -> None:
        """Display duplicate files warning"""
        self.console.print(f"[yellow][警告]  同名の画像ファイルが複数回参照されています:[/yellow]")
    ...
    def test_statistics(self, stats: dict, double_click_mode: bool = False) -> None:
        ...
        if double_click_mode:
            ...
            self.console.print(f"[dim]   [キーワード]  単一キーワード数: {stats['single_keywords']}[/dim]")
    ...
    def zip_start(self, source_path: str) -> None:
        ...
        self.console.print(f"[green][ZIP配布] 配布パッケージ作成開始:[/green] {source_path}")
    
    def zip_exclusion_loaded(self, pattern_count: int) -> None:
        """Display exclusion pattern loading"""
        self.console.print(f"[blue][除外設定] .distignoreから{pattern_count}個の除外パターンを読み込みました[/blue]")
    ...
    def zip_copy_complete(self, copied_count: int, excluded_count: int = 0) -> None:
        ...
        self.console.print(f"[green][コピー完了] {copied_count}個のファイルをコピー[/green]")
        if excluded_count > 0:
            self.console.print(f"[yellow][除外] {excluded_count}個のファイルを除外[/yellow]")
    ...
    def sample_complete(self, output_path: str, txt_name: str, html_name: str, image_count: int) -> None:
    ...
    def test_file_generation(self, double_click_mode: bool = False) -> None:
        ...
        if double_click_mode:
            ...
            self.console.print("[dim]   すべての記法パターンを網羅したテストファイルを作成します[/dim]")
        else:
            self.console.print("[cyan][設定] テスト用記法網羅ファイルを生成中...[/cyan]")
    ...
    def test_conversion_start(self, double_click_mode: bool = False) -> None:
        ...
        if double_click_mode:
            ...
            self.console.print("[dim]   すべての記法が正しく処理されるかテストしています[/dim]")
        else:
            self.console.print("\n[yellow][テスト] 生成されたファイルのテスト変換を実行中...[/yellow]")
    
    def test_conversion_complete(self, test_output_file: str, output_file: str, double_click_mode: bool = False) -> None:
        """Display test conversion completion"""
        if double_click_mode:
            self.console.print(f"[green][完了] HTML変換成功:[/green] {test_output_file}")
            self.console.print("[dim]   [ファイル] テストファイル (.txt) と変換結果 (.html) の両方が生成されました[/dim]")
