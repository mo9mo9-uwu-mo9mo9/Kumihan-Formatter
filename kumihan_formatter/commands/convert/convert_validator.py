...
class ConvertValidator:
        ...
        try:
            ...
            if results:
                for file_path, errors in results.items():
                    for error in errors:
                        # 旧形式から新形式に変換
                        detailed_error = self._convert_to_detailed_error(error, file_path)
    ...
    def _convert_to_detailed_error(self, error, file_path: Path):
        ...
        from ...core.reporting import DetailedError, ErrorSeverity, ErrorCategory, ErrorLocation
        
        return DetailedError(
            error_id=f"syntax_{error.line}_{hash(error.message)}",
            severity=ErrorSeverity.ERROR if error.severity == ErrorSeverity.ERROR else ErrorSeverity.WARNING,
    ...
    def save_error_report(self, error_report: ErrorReport, input_path: Path, output_dir: str) -> None:
        """エラーレポートをファイルに保存"""
        try:
