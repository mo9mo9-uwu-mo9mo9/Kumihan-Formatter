...
class ConvertCommand:
    ...
    def _handle_file_error(self, e: FileNotFoundError, input_file: Optional[str]) -> None:
        """ファイル未発見エラーの処理"""
    ...
    def _handle_encoding_error(self, e: UnicodeDecodeError, input_file: Optional[str]) -> None:
        """文字エンコーディングエラーの処理"""
        error = self.friendly_error_handler.handle_exception(
            e, context={"file_path": input_file or ""}
        )
    ...
    def _handle_permission_error(self, e: PermissionError, input_file: Optional[str]) -> None:
        """ファイル権限エラーの処理"""
        error = self.friendly_error_handler.handle_exception(
