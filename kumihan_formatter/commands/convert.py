...
from ..core.error_handling import ErrorCatalog
from ..core.error_handling import ErrorHandler as FriendlyErrorHandler
from ..core.error_reporting import DetailedError, ErrorReport, ErrorReportBuilder
from ..core.file_ops import ErrorHandler, FileOperations, PathValidator
...
class ConvertCommand:
        ...
        try:
            ...
            if watch:
                self._handle_watch_mode(
                    input_file, output, config_obj, show_test_cases, template_name, include_source, syntax_check
                )

            return output_file

        except FileNotFoundError as e:
            error = self.friendly_error_handler.handle_exception(e, context={"file_path": input_file or ""})
    ...
    def _parse_with_progress(self, text: str, config) -> Any:
        ...
        with Progress() as progress:
            ...
            if size_mb > 10:  # 10MB以上
                task = progress.add_task(f"[cyan]大規模ファイルを解析中 ({size_mb:.1f}MB)", total=100)
            else:
    ...
    def _render_with_progress(
        ...
        with Progress() as progress:
            ...
            if node_count > 1000:  # 1000ノード以上
                task = progress.add_task(f"[yellow]大規模データを変換中 ({node_count:,}要素)", total=100)
            else:
    ...
    def _show_conversion_stats(self, ast: Any, text: str, output_file: Path, input_path: Path) -> None:
        ...
        stats = {
            ...
            "compression_ratio": (
                output_size_info["size_mb"] / input_size_info["size_mb"] if input_size_info["size_mb"] > 0 else 0
            ),
        }
        ...
        if input_size_info["is_large"]:
            ui.dim(
                f"入力ファイル: {input_size_info['size_mb']:.1f}MB → 出力ファイル: {output_size_info['size_mb']:.1f}MB"
            )
    ...
    def _convert_syntax_error_to_detailed(self, syntax_error, input_path: Path) -> DetailedError:
        ...
        if hasattr(syntax_error, "suggestion") and syntax_error.suggestion:
            suggestions.append(FixSuggestion(description=syntax_error.suggestion, confidence=0.8))
                ...
                if line_stripped.startswith(";;;") and line_stripped != ";;;":
                    ...
                    if not found_end:
                        error = ErrorReportBuilder.create_syntax_error(
                            ...
                            suggestions=[
                                FixSuggestion(
                                    description="ブロックの最後に ;;; を追加する", action_type="insert", confidence=0.9
                                )
                            ],
                        )
                        error_report.add_error(error)
...
def create_convert_command():
    ...
    def convert(
        input_file, output, no_preview, watch, show_test_cases, with_source_toggle, no_syntax_check, experimental
    ):
        """テキストファイルをHTMLに変換します"""
