...
class ConvertCommand:
        ...
        try:
            ...
            if error_count > 0:
                ...
                if sum(len(errors) for errors in results.values()) > 3:
                    ui.dim(
                        f"  ... 他 {sum(len(errors) for errors in results.values()) - 3} 個の問題"
                    )
                ...
                ui.warning(
                    "記法警告",
                    f"{warning_count} 個の警告がありますが、変換を続行します",
                )
                return True
    ...
    def _show_conversion_stats(
        ...
        stats = {
            ...
            "compression_ratio": (
                output_size_info["size_mb"] / input_size_info["size_mb"]
                if input_size_info["size_mb"] > 0
                else 0
            ),
        }
        ...
        if input_size_info["is_large"]:
            ...
            if stats["compression_ratio"] > 1:
                ui.hint(
                    f"HTML変換により約{stats['compression_ratio']:.1f}倍のサイズになりました"
                )
        ...
        try:
            ...
            for file_path_str, syntax_errors in results.items():
                for syntax_error in syntax_errors:
                    ...
                    detailed_error = self._convert_syntax_error_to_detailed(
                        syntax_error, input_path
                    )
    ...
    def _convert_syntax_error_to_detailed(
        self, syntax_error, input_path: Path
    ) -> DetailedError:
        ...
        if hasattr(syntax_error, "suggestion") and syntax_error.suggestion:
            suggestions.append(
                FixSuggestion(description=syntax_error.suggestion, confidence=0.8)
            )
...
def create_convert_command():
    ...
    @click.option(
        "--no-syntax-check", is_flag=True, help="変換前の記法チェックをスキップ"
    )
    @click.option(
        "--experimental", type=str, help="実験的機能を有効化 (例: scroll-sync)"
    )
    def convert(
        input_file,
        output,
