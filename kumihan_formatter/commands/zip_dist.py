...
class ZipDistCommand:
    ...
    def _process_files(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if convert_docs:
                ...
                ui.info(
                    f"文書変換完了: HTML {stats['converted_to_html']}件, TXT {stats['converted_to_txt']}件"
                )
            else:
            ...
            # Create final output
            if no_zip:
                self._create_directory_output(
                    temp_path, output_path, zip_name, no_preview
                )
            else:
                self._create_zip_output(temp_path, output_path, zip_name, no_preview)

    def _copy_source_files(
        self, source_path: Path, temp_path: Path, exclude_patterns: list
    ) -> None:
    ...
    def _create_directory_output(
        self, temp_path: Path, output_path: Path, zip_name: str, no_preview: bool
    ) -> None:
    ...
    def _show_zip_preview(
        self, zip_file_path: Path, output_path: Path, zip_name: str
    ) -> None:
...
def create_zip_dist_command():
    ...
    @click.option(
        "-o", "--output", default="zip_distribution", help="ZIP配布用出力ディレクトリ"
    )
    @click.option(
        "--zip-name",
        default="kumihan-formatter-distribution",
        help="ZIPファイル名（拡張子なし）",
    )
    @click.option(
        "--no-zip", is_flag=True, help="ZIPファイルを作成せず、ディレクトリのみ作成"
    )
    @click.option(
        "--no-preview", is_flag=True, help="生成後にブラウザでプレビューしない"
    )
    @click.option(
        "--no-convert-docs",
        is_flag=True,
        help="文書変換をスキップ（従来の単純コピー方式）",
    )
    @click.option(
        "--include-dev-docs", is_flag=True, help="開発者向け文書も配布に含める"
    )
    def zip_dist(
        source_dir,
        output,
