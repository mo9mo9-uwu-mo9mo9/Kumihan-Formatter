...
def main():
    ...
    # チャートの種類を選択
    chart_type = st.sidebar.selectbox("チャートの種類", ["線グラフ", "エリアチャート", "棒グラフ"])
        ...
        while iteration < max_iterations:
            try:
                ...
                with placeholder.container():
                    st.subheader(f"📊 データ可視化 (更新: {iteration + 1}/{max_iterations})")
                    ...
                    with col3:
                        st.metric("🎯 フィルタ適用", f"{filter_column} > {filter_threshold}")
                ...
                with stats_placeholder.container():
                    ...
                    with stats_col2:
                        ...
                        for column, trend in trends.items():
                            direction = "📈 上昇" if trend > 0 else "📉 下降" if trend < 0 else "➡️ 横ばい"
