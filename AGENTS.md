...
### Communication & Language Policy（言語ポリシー）
- 本プロジェクトでのやりとり（Issue/PRのタイトル・本文、コメント、レビュー、設計メモ、やりとりの文章）は原則すべて日本語で行うこと。
- 例外: コード、識別子（関数名・クラス名・変数名等）、ファイル名は英語（既存規約どおり）。
- エージェント/ボットの応答既定言語も日本語とし、特段の指定がない限り英語返信は禁止。
