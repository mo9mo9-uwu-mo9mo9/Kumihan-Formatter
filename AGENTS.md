...
### Automation Mode（自走モード）
- 目的: 軽微～中規模の保守作業（Deprecation対応、ドキュメント整備、テスト/品質、Issue運用）を「確認なし」で安全に自動実施する。
- 発動フレーズ（例）:
  - 「自走モード開始。確認なしで最後まで。」（既定）
  - 「自走でP0/P1だけ。Deprecation系に限定。」（スコープ指定）
  - 「#1303→#1304→#1305の順で確認不要で。」（順序指定）
  - 「自走ドライラン。PRは作るがマージはしない。」（途中停止）
- 既定運用（未指定時）:
  - 対象: 既存Issueのみ（優先度 P0→P1→P2）。新機能・大規模設計変更は除外。
  - 許可操作: ghでのIssue/PR作成・更新・squashマージ・ブランチ削除、ローカル編集、`make lint` / 必要最小のテスト実行。
  - 単位: 1 Issue = 1 PR。最小差分、コミット文は規約（絵文字＋種別＋Issue番号）。
  - 報告: PR作成時・マージ時のみ簡潔に共有（詳細ログは `tmp/automation-logs/`）。
- 品質ゲート:
  - 変更ごとに `make lint` を必須化（Black+mypy strict）。
  - テストは変更に関連する最小セット＋必要に応じ全体。失敗が既知で非関連なら影響を分離。
  - ドキュメント/設定の整合は `scripts/document_consistency_checker.py` で確認。
- 要確認/禁止（該当時は一度だけ確認）:
  - 強制push/履歴改変、リリース/タグ操作、ライセンス・法務関連変更、互換性破壊の大規模改変、大量一括削除。
- 停止/一時停止フレーズ:
  - 「停止」「一時停止」「自走オフ」で即停止。
  - 「セーフモード」= PR までは作成、マージは保留。
