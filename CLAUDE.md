# CLAUDE.md

> **Kumihan-Formatter** - Claude Code プロジェクト設定ファイル  
> **Status**: Development - 2025年版最適化済み

---

## 🎯 プロジェクト概要

- **言語**: 日本語メインプロジェクト
- **技術スタック**: Python 3.12+, Black, isort, mypy (strict)
- **エンコーディング**: UTF-8
- **記法**: Kumihan独自ブロック記法 (`# 装飾名 #内容##`)

## 📋 開発原則

### コア原則
1. **作業計画の事前報告**: ファイル変更・実行前に必ず計画を報告
2. **ユーザー主導**: AIは提案のみ、決定権はユーザーが保持
3. **指示の厳格遵守**: 非効率でもユーザー指示を最優先
4. **透明性の確保**: 全作業プロセスを明確に報告
5. **🤖 Gemini協業優先**: Token節約・コスト効率化のため積極活用

### 基本ルール
- **新規Issue対応時のみ**: 適切なブランチ作成・切り替え実施
- **一時ファイル**: 全て `tmp/` 配下に出力（絶対遵守）
- **日本語使用**: コメント・レビュー・ドキュメントは日本語
- **ログ使用**: `from kumihan_formatter.core.utilities.logger import get_logger`
### 🤖 Gemini協業指針
- **直接協業**: Task toolでGeminiに作業指示を直接送信
- **使用モデル**: gemini-2.5-flash固定（コスト最適化）
- **トリガーワード**: 幅広い協業表現を認識
  - 「Geminiと協業」「Gemini協業」
  - 「Geminiと一緒に」「Geminiで実装」
  - 「Geminiに依頼」「Geminiと連携」
  - 「協業で」「一緒に作って」
- **指示書作成**: 詳細な作業指示を`tmp/`配下に作成してGeminiに渡す
- **Token節約目標**: 90%以上（Claude使用を最小限に）
- **品質責任**: Claude最終責任で品質保証

#### 🤖 協業対象パターン
```python
PATTERNS = {
    "SIMPLE": ["mypy", "flake8", "black", "isort", "lint", "型注釈"],
    "MODERATE": ["機能追加", "バグ修正", "テスト実装"],
    "COMPLEX": ["アーキテクチャ", "大規模リファクタリング"]
}
# 👑 Claude: 要件分析・設計・指示書作成・品質レビュー
# 🤖 Gemini: 指示書に従った実装のみ
```

---

## 🔧 開発ワークフロー

### ブランチ管理
```bash
# 必須形式: {type}/issue-{番号}-{英語概要}
git checkout -b feat/issue-123-description
```

### Issue・PR管理
```bash
# Issue作成（ラベル必須）
gh issue create --title "タイトル" --body "内容" \
  --label "バグ,優先度:高,難易度:普通,コンポーネント:パーサー"

# PR作成
gh pr create --title "タイトル" --body "詳細説明"
```

### テスト・品質管理
```bash
make lint       # Black, isort, flake8, mypy
make test       # pytest
```

#### 品質管理基盤 (Issue #831対応済み)
- **設定統一**: pre-commit ↔ mypy 完全統一（pyproject.toml基準）
- **現状**: 73件エラー（段階的解決計画策定済み）
- **後続対応**: Issue #832(unreachable), #833(return-value)で段階的改善

---

## 👑 Claude-Gemini 直接協業体制

> **Claude(PM/Manager) - Gemini(Coder)** - シンプルで確実な90%Token削減

### Claude(PM/Manager)責任範囲
- **📋 要件分析・設計**: ユーザー要求の分析・アーキテクチャ設計・詳細作業指示書作成
- **👀 監督・レビュー**: Gemini成果物の品質チェック・最終調整・品質責任
- **👥 コミュニケーション**: ユーザー対話・意思決定・問題解決

### Gemini(Coder)責任範囲
- **👤 指示実行**: Task toolを通じて受け取る作業指示書の忠実な実行
- **⚡ 実装作業**: コード修正・型注釈・バグ修正・コード整形・テスト実行
- **🚫 判断禁止**: 独自判断・仕様変更・品質基準の変更は禁止

### 💰 Token節約戦略（目標: 90%削減）
- **モデル選択**: gemini-2.5-flash使用でコスト最適化
- **シンプル実行**: Task tool 1回呼び出しで完了
- **コスト効率**: 複雑なシステム不要で真のコスト削減
- **品質維持**: Claude最終責任で品質保証

### 📋 新作業フロー
```bash
1. ユーザー要求受付
   ↓
2. 👑 Claude: 要件分析・詳細作業指示書作成（tmp/配下）
   ↓
3. 👑 Claude: Task toolでGeminiに直接指示
   ↓
4. 🤖 Gemini: 指示書に従って実装のみ
   ↓
5. 👑 Claude: 品質レビュー・最終調整
   ↓
6. 完成
```

---

## 🛡️ 品質保証システム（3層検証体制）

### Layer 1: 構文検証（自動）
- AST解析による構文エラー検出、型注釈パターン自動修正、禁止構文の事前検証

### Layer 2: 品質検証（自動）
```bash
make lint        # Black, isort, flake8 通過必須
make mypy        # strict mode 通過必須  
make test        # 既存テスト全通過必須
```

### Layer 3: Claude最終承認（手動）
- **コードレビュー**: Gemini成果物の詳細確認
- **品質責任**: 最終的な品質保証・承認
- **統合確認**: 全体システムとの整合性検証

### 🔄 フェイルセーフ機能
- Gemini実行失敗時: 自動的にClaude代替実行
- 品質基準未達時: 自動的な追加修正指示
- エラー時: 詳細ログ記録・学習データ化

### 📊 品質監視
- **コード品質**: `make lint` / `make test` で確認
- **品質履歴追跡**: Git履歴による改善トレンド分析
- **コスト・品質バランス**: シンプルな直接協業によるROI最適化

---

## 📝 コーディング標準

### Python コードスタイル
- **フォーマット**: Black (line-length=88)
- **インポート**: isort設定に従う
- **型注釈**: mypy strict mode必須
- **ログ**: プロジェクト標準ロガー使用

### ファイル出力規則
```python
# 正しい実装パターン
output_path = "tmp/" + filename
os.makedirs("tmp", exist_ok=True)
with open(output_path, "w", encoding="utf-8") as f:
    f.write(content)
```

### エラーハンドリング
```python
# 推奨パターン
from kumihan_formatter.core.utilities.logger import get_logger
logger = get_logger(__name__)

try:
    # 処理
    pass
except Exception as e:
    logger.error(f"エラー詳細: {e}")
    raise
```

---

## 🎨 Kumihan記法仕様

### 基本記法
- **ブロック記法**: `# 装飾名 #内容##` （太字・イタリック・見出し等）
- **構造要素**: 見出し・リスト・目次・リンク対応
- **詳細仕様**: [記法完全仕様](docs/specs/notation.md)

---

## 📊 プロジェクト構造

### 主要コンポーネント
- **パーサー**: `core/` - 記法解析エンジン（block_parser, utilities含む）
- **レンダラー**: `core/rendering/` - HTML出力・テンプレート処理
- **CLI**: `cli.py + commands/` - コマンドライン interface  
- **設定**: `config/` - 設定管理・品質基準

---

## 🔍 品質・性能指標

### CLAUDE.md サイズ管理
- **推奨**: 150行/8KB以下（品質重視）
- **警告**: 200行/10KB（見直し推奨）
- **クリティカル**: 250行/12KB（即座対応必須）

### 監視・ドキュメント
- **監視**: `make claude-check` / `make code-quality` （品質・性能評価）
- **開発者向け**: [アーキテクチャ](docs/dev/architecture.md) / [Gemini協業](docs/claude/gemini-collaboration.md)  
- **記法・仕様**: [記法仕様](docs/specs/notation.md) / [機能仕様](docs/specs/functional.md)
- **ユーザー**: [利用ガイド](docs/user/user-guide.md) / [Claude活用法](docs/claude/reference.md)

---

## ⚙️ 重要な実行時注意事項

### 禁止事項
- ❌ 日本語ブランチ名（システム的に拒否）
- ❌ プロジェクトルート直下への一時ファイル出力
- ❌ 英語でのレビュー・コメント
- ❌ 未承認でのファイル作成・変更
- ❌ **`python`コマンド使用**（必ず`python3`を使用）
- ❌ **Gemini品質基準未達成でのマージ**（3層検証必須）

### 必須事項
- ✅ 作業前の計画報告
- ✅ tmp/ 配下での一時ファイル管理
- ✅ 適切なラベル付きIssue作成
- ✅ 品質チェック（lint/typecheck）の実行
- ✅ **🤖 Gemini協業の明示実行**（「Geminiと一緒に」等の協業表現で自動起動）
- ✅ **3層品質検証**: 構文→品質→Claude最終承認
- ✅ **Token節約1000以上時Gemini検討**（コスト効率優先）

### 🤖 Gemini協業時の特別注意事項
- **品質責任**: Gemini成果物でもClaude最終責任
- **シンプル実行**: Task toolによる直接指示で確実性重視
- **継続改善**: 結果蓄積・学習データ化・品質向上

---

## 🛠️ Gemini協業品質保証ルール（Issue #920改善対応）

### 🔍 必須品質チェック（Claude責任）
**Gemini協業完了後、必ず以下を実行:**

```bash
# 1. 基本品質チェック（必須・自動実行）
make lint              # Black, isort, flake8通過必須
make test              # 既存テスト全通過必須
git status            # 意図しない変更確認

# 2. 拡張品質チェック（推奨）
make gemini-quality-check    # 統合品質チェック
make gemini-post-review      # 協業後レビュー
```

### ✅ Gemini協業完了判定基準
- [ ] lint/flake8エラー: 0件
- [ ] 既存テスト: 全通過
- [ ] import文最適化: 完了
- [ ] API互換性: 維持確認
- [ ] ドキュメント更新: 必要箇所完了
- [ ] 統合テスト: 実行・通過
- [ ] セキュリティチェック: 問題なし

### 🚨 作業漏れ防止チェックリスト
**以下が未完了の場合、協業を継続:**
1. **機能テスト**: 分割・変更したコードの動作確認
2. **依存関係**: import文・呼び出し関係の整合性
3. **エラーハンドリング**: 例外処理の適切な実装
4. **型注釈**: mypy対応型注釈の配置
5. **ログ出力**: 必要箇所への適切なログ記述

### 📋 段階的確認ポイント
```
Phase 1 (33%完了): 基本実装
→ 中間品質チェック: make lint
→ 問題があれば即座に修正指示

Phase 2 (66%完了): 統合・テスト
→ 中間品質チェック: make test
→ API互換性確認

Phase 3 (100%完了): 最終確認
→ 最終品質チェック: 全項目確認
→ Claude最終承認
```

### 🎯 指示書品質基準
**Geminiへの指示書は必ず以下を含む:**
1. **具体的成果物定義**: 「何を作るか」明確化
2. **品質基準**: lint/flake8/test通過必須を明記
3. **確認テスト項目**: 動作確認すべき項目リスト
4. **完了判定基準**: 明確な完了条件
5. **自己チェック項目**: Gemini自身で確認すべき事項

### 📊 品質メトリクス追跡
- **協業成功率**: 一発完了 vs 修正必要
- **品質スコア**: エラー件数・修正工数
- **効率指標**: Token節約率 vs 品質維持率
- **学習データ**: 失敗パターン・改善点蓄積

---

## 🚀 Gemini協業運用ルール（実務手順）

### 📋 協業開始前チェック
**Claude必須実行事項:**
1. **指示書作成**: `docs/claude/gemini_instruction_template.md`を使用
2. **品質基準明記**: lint/flake8/test通過を明確化
3. **完了判定基準**: 具体的な成果物・確認項目設定
4. **段階確認設定**: 33%/66%/100%時点でのチェックポイント

### ⚡ 協業実行中ルール
**中間確認必須:**
```bash
# Phase 1完了時 (33%)
make lint  # エラー0件確認

# Phase 2完了時 (66%) 
make test  # 既存テスト通過確認

# Phase 3完了時 (100%)
make gemini-quality-check  # 統合品質チェック
```

### ✅ 協業完了後必須手順
**Claude責任で以下を順次実行:**
```bash
# 1. 基本品質保証（必須・即時実行）
make gemini-quality-check
→ lint/flake8/test/import確認

# 2. 総合レビュー（推奨・詳細確認）
make gemini-post-review  
→ セキュリティ・パフォーマンス・依存関係・ドキュメント・保守性

# 3. 完全バリデーション（重要作業時）
make gemini-validation-full
→ 3層検証システム完全実行
```

### 🚨 品質問題発生時の対応
```bash
品質チェック失敗
↓
問題分類: Critical/Warning/Info
↓
Critical → Geminiに即座修正指示
Warning → 状況判断後、Gemini修正 or Claude修正
Info → 将来Issue作成・継続作業
```

### 📊 継続改善システム
- **成功パターン分析**: 一発合格時の指示書特徴記録
- **失敗パターン学習**: 修正必要時の共通問題蓄積
- **テンプレート改良**: 指示書・チェックリストの段階的改善
- **自動化推進**: 品質チェックの更なる自動化

### 🎯 協業効率最適化指針
1. **Token節約 vs 品質維持**: 50%節約・95%品質維持目標
2. **ハイブリッド分担**: Claude(設計・監督) + Gemini(実装)
3. **段階的品質保証**: 早期発見・即座修正で後戻り最小化
4. **継続学習**: 各協業から得た知見を次回に活用

---

*🎯 Claude Code最適化済み - Issue #920改善対応版*