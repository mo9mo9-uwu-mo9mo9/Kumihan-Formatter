...
3. **作業用ブランチ作成**: `git checkout -b feature/作業内容` で新ブランチを作成
4. **PR必須**: 作業完了後は必ずプルリクエストを作成
5. **Claude自身がPRレビューを実行**: 人間より優れているため、Claude自身が必ずレビューを行う（絶対遵守）
6. **レビュー後にマージ**: レビュー完了後にmainブランチにマージ
7. **Issue更新の実行**: マージ後には作業内容に適したIssue管理を必ず実行（絶対遵守）
   - Issue作成（新機能・バグ報告等）
   - Issue Close（作業完了時）
   - Issue削除（不要になった場合）
...
- **設定ファイル機能は非推奨**: 初心者には過剰に複雑で、本来の目標「開発知識なしで使える」と矛盾
- **カスタムマーカー機能は削除予定**: YAML/CSS知識が必要で初心者向けではない
- **固定記法セットに集中**: 見出し、太字、枠線、ハイライト等の基本記法のみ
- **ゼロ設定で即使用可能**: 設定作業なしで「そのまま使える」ことを最優先
- **学習コスト最小化**: 覚える記法を最小限に絞り、5分で最初の変換成功を目標

### 📅 **更新履歴**
- 2025-06-26: ワークフロー改善とセットアップファイル移動（現在）
