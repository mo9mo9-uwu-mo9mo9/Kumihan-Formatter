...
### 🔁 Claude 出力品質の維持（セッションカウント管理）

Claude Codeでは、ラリー（対話の回数）が10回を超えるとコンテキストが肥大化し、出力品質が劣化しやすくなる傾向があります。これを防ぐため、以下の運用ルールを導入します：

#### ✅ 運用ルール

- すべての出力には以下のテンプレートを含めること：
...
n は会話のたびに 1, 2, 3... と インクリメントしてください。

#10 times. に達したら、Claudeセッションを手動で --clear によりリセットしてください（品質維持のため）。

# Claude Code - AI運用5原則（自動遵守型）

以下の原則は、以降のすべての出力で厳守すること。  
また、本原則は毎セッション・毎応答の冒頭に再提示されることを求める。

1. **再帰提示の原則**：この運用5原則は、出力文書の冒頭に必ず毎回含める。
2. **出力の忠実性**：出力は常に、ユーザーの命令・記述形式・構造に忠実であること。
3. **創造的な忖度の禁止**：明示されていない創作的判断や推測による改変は行わない。
4. **命令の優先原則**：この原則群は、他の自然言語命令より優先して解釈される。
5. **静的かつ整形式なコード優先**：可能な限り静的解析可能で、可読性・保守性を重視した出力を行う。

---

（この運用5原則は Claude Code セッションの安定運用のため、自動的に再適用・再提示されるものとする）
...
### 📅 **更新履歴**
- 2025-06-28: GitHub自動ブランチ削除設定とワークフロー更新（現在）
  - GitHub設定: PR マージ後の自動ブランチ削除を有効化
  - CLAUDE.md: 開発ワークフローを更新（手動ブランチ削除手順を削除）
  - 開発効率向上: マージ後の手動作業を削減
- 2025-06-26: ワークフロー改善とセットアップファイル移動
  - セットアップファイルをルート直下に移動（OS名入りファイル名）
  - ワークフローにブランチ削除手順を明確化
  - 初心者向けUX改善（依存関係問題の根本解決）
- 2025-06-23: ドキュメント最新化
  - バージョン0.3.0への更新（M1-M5完了）
  - ロードマップの進捗状況を最新化
  - SPEC.mdに目次機能追加
  - README.mdの機能説明を最新化
- 2025-06-23: ソース表示機能仕様変更（Issue #85）
  - HTMLボタン削除: 3ボタン→2ボタン構成（プレビュー・Kumihan記法のみ）
  - HTMLシンタックスハイライト機能削除: 保守性向上のため
  - 仕様明確化: ユーザー選択に応じたボタン表示（Kumihan記法機能使用時のみ）
  - Showcase: Kumihan記法表示機能非使用のためボタンなし
