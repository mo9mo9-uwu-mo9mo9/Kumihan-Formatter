...
### 各チャット開始時の必須チェック
- [ ] **第5原則**: AI運用7原則を逐語的に画面出力した
- [ ] **違反監視**: 前回チャット終了時に違反がないことを確認した
- [ ] **状態継続**: TDD状態・Issue状態・ブランチ状態を正確に把握した

### 作業開始前の必須チェック
- [ ] **第1原則**: 作業計画を明確に報告した
- [ ] **第6原則**: Issue番号とブランチ名の一致を確認した
- [ ] **第7原則**: TDDセッション状態を確認した
- [ ] **権限確認**: ユーザーからの明確な指示を受けた

### 作業実行中の必須チェック
- [ ] **第3原則**: ユーザーの指示通りに実行している（最適化・勝手な判断なし）
- [ ] **第7原則**: TDD-First開発システムを厳格に遵守している
- [ ] **フェーズ遵守**: Red→Green→Refactorの順序を守っている
- [ ] **違反検出**: TDDサイクル違反時は即座に作業停止している

### 計画変更・問題発生時の必須チェック
- [ ] **第2原則**: 勝手な迂回・別アプローチを行わず、ユーザーに確認を取った
- [ ] **第4原則**: ルールの歪曲・解釈変更を行わなかった
- [ ] **是正措置**: 違反検出時は即座に是正措置を実行した
- [ ] **報告義務**: 問題状況をユーザーに正確に報告した

### 作業完了時の必須チェック
- [ ] **完了確認**: 指示された作業が完全に完了した
- [ ] **品質確認**: TDD品質基準をクリアした
- [ ] **違反なし**: 全プロセスで原則違反がなかった
- [ ] **記録保持**: 作業履歴・TDD履歴が正確に記録された

## ⚠️ 緊急停止条件（即座に作業停止）

以下の状況を検出した場合、AIは即座に全作業を停止し、違反報告・是正措置を実行する：

1. **第1原則違反**: 作業計画報告なしにファイル生成・更新・プログラム実行を試行
2. **第2原則違反**: ユーザー確認なしに勝手な迂回・別アプローチを実行
3. **第3原則違反**: ユーザー指示を無視して独自判断・最適化を実行
4. **第5原則違反**: チャット開始時に原則出力を省略・簡略化
5. **第6原則違反**: Issue番号とブランチ名不一致を放置して作業継続
6. **第7原則違反**: TDDサイクル違反（順序破り・工程スキップ・未実行継続）
...
## 🛡️ チャット開始時の絶対遵守チェックリスト
- [ ] **第5原則確認**: 上記7原則を逐語的に出力完了
- [ ] **TDD状態確認**: `ls .tdd_session.json` でTDDセッション状態確認
- [ ] **違反検出**: 前回終了時の違反がないことを確認
- [ ] **緊急停止準備**: 違反検出時の即座停止体制確認
