...
class TDDSpecGenerator:
    ...
    def generate_specifications(self) -> SpecificationSet:
        ...
        logger.info(
            f"✅ TDD仕様テンプレート生成完了: {len(specifications)}個のテスト仕様"
        )
        return spec_set
    ...
    def _generate_test_specifications(
        self, analysis: Dict, naming: Dict
    ...
    def _determine_file_paths(self, naming: Dict) -> Dict:
        ...
        if feature_type in ["parser", "renderer"]:
            impl_dir = (
                self.project_root / "kumihan_formatter" / "core" / f"{feature_type}s"
            )
        elif feature_type == "command":
        ...
        return {"implementation_file": impl_file, "test_file": test_file}
...
'''
        ...
        if not spec_set.implementation_file_path.exists():
            ...
            logger.info(
                f"⚠️  実装ファイル既存のためスキップ: {spec_set.implementation_file_path}"
            )
...
def main():
    ...
    if spec_set:
        ...
        print(
            f"📚 仕様書: scripts/tdd_templates/TDD_SPEC_Issue_{spec_set.issue_number}.md"
        )
        print()
        print("次のステップ:")
