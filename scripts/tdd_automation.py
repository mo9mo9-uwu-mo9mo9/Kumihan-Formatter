...
class TDDAutomation:
    ...
    def _file_watcher(self):
        ...
        while self.running:
            try:
                ...
                current_timestamps = {f: f.stat().st_mtime for f in current_watched if f.exists()}
    ...
    def _find_related_tests(self, file_path: Path) -> List[Path]:
        ...
        if file_path.parent.name != "kumihan_formatter":
            package_tests = self.project_root.glob(f"tests/**/test_{file_path.parent.name}*.py")
    ...
    def _process_test_result(self, result: subprocess.CompletedProcess, duration: float, test_type: str):
        """テスト結果処理"""
    ...
    def _send_notification(self, success: bool, test_type: str, summary: str, duration: float):
        """通知送信"""
