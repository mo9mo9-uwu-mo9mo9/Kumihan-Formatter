...
class CSRFTester(TDDSystemBase):
    ...
    def _check_flask_routes(self, file_path: Path, content: str):
        ...
        for pattern, description in self.flask_route_patterns.items():
            for match in re.finditer(pattern, content, re.IGNORECASE):
                ...
                methods_str = match.group(2) if match.groups() and len(match.groups()) > 1 else ""
                ...
                for method in methods:
                    if method in self.state_changing_methods:
                        ...
                        # CSRF保護があるかチェック
                        has_protection = self._check_route_csrf_protection(content, match)
                        
                        if not has_protection:
                            vulnerability = self._create_csrf_vulnerability(
                                file_path, line_num, "flask_route", route_path,
                                method, f"Flask route without CSRF protection: {method} {route_path}",
                                match.group(), has_protection
                            )
    ...
    def _check_django_views(self, file_path: Path, content: str):
        ...
        for match in re.finditer(view_function_pattern, content):
            ...
            if re.search(r'request\.method\s*==\s*["\']POST["\']', func_content):
                ...
                if not has_protection:
                    vulnerability = self._create_csrf_vulnerability(
                        file_path, line_num, function_name, f"/{function_name}",
                        "POST", f"Django view without CSRF protection: {function_name}",
                        match.group(), has_protection
                    )
                    self.vulnerabilities.append(vulnerability)
    ...
    def _check_form_csrf_protection(self, file_path: Path, content: str):
        ...
        for pattern, description in self.form_patterns.items():
            for match in re.finditer(pattern, content, re.IGNORECASE | re.DOTALL):
                ...
                form_end_match = re.search(r'</form>', content[match.end():], re.IGNORECASE)
                
                if form_end_match:
                    # フォーム全体を取得
                    full_form = content[match.start():match.end() + form_end_match.end()]
                    ...
                    if not has_csrf_token:
                        vulnerability = self._create_csrf_vulnerability(
                            file_path, line_num, "html_form", "form",
                            "POST", f"Form without CSRF token: {description}",
                            form_content[:100], False
                        )
                        self.vulnerabilities.append(vulnerability)
        ...
        for pattern, description in self.ajax_patterns.items():
            for match in re.finditer(pattern, content, re.IGNORECASE):
                ...
                if not has_csrf_header:
                    vulnerability = self._create_csrf_vulnerability(
                        file_path, line_num, "ajax_request", "ajax",
                        "POST", f"Ajax request without CSRF protection: {description}",
                        match.group(), False
                    )
                    self.vulnerabilities.append(vulnerability)
    ...
    def _create_csrf_vulnerability(self, file_path: Path, line_number: int,
                                 func_name: str, endpoint_path: str, http_method: str,
                                 description: str, code_snippet: str,
                                 has_protection: bool) -> CSRFVulnerability:
        """CSRF脆弱性オブジェクト作成"""
        
        # リスクレベル評価
        risk_level, severity_score = self._assess_csrf_risk(http_method, endpoint_path, has_protection)
    ...
    def _assess_csrf_risk(self, http_method: str, endpoint_path: str, has_protection: bool) -> Tuple[CSRFRisk, float]:
        """CSRFリスクレベル評価"""
        base_score = 0.0
    ...
    def _generate_csrf_recommendation(self, http_method: str, func_name: str) -> str:
        ...
        recommendation = base_recommendations.get(func_name, 
            '状態変更操作にはCSRF保護を実装してください')
    ...
    def _generate_security_report(self, result: CSRFTestResult):
        ...
        report_data = {
            ...
            "risk_distribution": self._get_csrf_risk_distribution(result.vulnerabilities_found),
            "method_distribution": self._get_method_distribution(result.vulnerabilities_found)
        }
    ...
    def _get_csrf_risk_distribution(self, vulnerabilities: List[CSRFVulnerability]) -> Dict[str, int]:
        """CSRFリスク分布取得"""
        distribution = {}
    ...
    def _get_method_distribution(self, vulnerabilities: List[CSRFVulnerability]) -> Dict[str, int]:
        """HTTPメソッド分布取得"""
        distribution = {}
        for vuln in vulnerabilities:
            method = vuln.http_method
...
def main():
    ...
    try:
        with CSRFTester(project_root) as tester:
            ...
            logger.info(f"  保護されたエンドポイント: {result.protected_endpoints_count}")
            ...
            if critical_vulns:
                ...
                for vuln in critical_vulns[:5]:  # 上位5件表示
                    logger.warning(f"  - {vuln.endpoint_path} ({vuln.http_method}) in {vuln.file_path}:{vuln.line_number}")
