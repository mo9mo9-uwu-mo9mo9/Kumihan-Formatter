...
class FileUploadTester(TDDSystemBase):
    ...
    def __init__(self, project_root: Path):
        ...
        if patterns_file.exists():
            with open(patterns_file, 'r', encoding='utf-8') as f:
                ...
                # 危険なファイル拡張子
                self.dangerous_extensions = set(file_upload_patterns.get('dangerous_extensions', []))
                
                # ファイルアップロード処理パターン
                self.upload_handler_patterns = {
                    pat_info['pattern']: pat_info['description'] 
                    for pat_info in file_upload_patterns.get('upload_handler_patterns', {}).values()
                }
                
                # セキュリティ検証パターン
                self.security_validation_patterns = {
                    pat_info['pattern']: pat_info['description'] 
                    for pat_info in file_upload_patterns.get('security_validation_patterns', {}).values()
                }
                
                # 悪意のあるファイルパターン
                self.malicious_file_patterns = {
                    pat_info['pattern']: pat_info['description'] 
                    for pat_info in file_upload_patterns.get('malicious_file_patterns', {}).values()
                }
        else:
        ...
        # ファイルサイズ制限
        self.max_file_size = security_config.get('file_upload', {}).get('max_file_size', 5242880)
    ...
    def execute_main_operation(self) -> FileUploadTestResult:
        ...
        try:
            ...
            logger.info(f"✅ ファイルアップロードテスト完了: {len(self.vulnerabilities)}件の脆弱性発見")
            return result
            ...
            for match in re.finditer(pattern, content, re.IGNORECASE):
                ...
                if security_checks['missing_validations']:
                    vulnerability = self._create_upload_vulnerability(
                        file_path, line_num, "file_upload_handler",
                        description, match.group(), security_checks
                    )
    ...
    def _analyze_security_validations(self, context: str) -> Dict:
        ...
        for required in required_validations:
            if not any(required.lower() in found.lower() for found in found_validations):
        ...
        return {
            ...
            'has_path_traversal_protection': '".." not in' in context or 'secure_filename' in context,
    ...
    def _run_malicious_file_tests(self):
        ...
        for test_name, test_data in malicious_tests.items():
            try:
                ...
                logger.debug(f"悪意ファイルテスト {test_name}: {'PASS' if result else 'FAIL'}")
            except Exception as e:
    ...
    def _simulate_file_validation(self, filename: str, file_path: Path, content: bytes) -> Dict:
        """ファイル検証シミュレーション"""
        blocked_reasons = []
        ...
        if len(content) > self.max_file_size:
            blocked_reasons.append(f"File too large: {len(content)} bytes > {self.max_file_size}")
    ...
    def _create_upload_vulnerability(self, file_path: Path, line_number: int,
                                   func_name: str, description: str,
                                   code_snippet: str, security_analysis: Dict) -> FileUploadVulnerability:
        ...
        return FileUploadVulnerability(
            ...
            dangerous_extensions=security_analysis.get('dangerous_extensions_allowed', []),
            missing_validations=security_analysis.get('missing_validations', [])
        )
    
    def _assess_upload_risk(self, security_analysis: Dict) -> Tuple[FileUploadRisk, float]:
        """ファイルアップロードリスクレベル評価"""
        base_score = 0.0
    ...
    def _generate_upload_recommendation(self, security_analysis: Dict) -> str:
        ...
        if 'MIME type validation' in missing:
            recommendations.append("MIMEタイプ検証とファイルマジックナンバーチェックを追加してください")
        
        if 'File size check' in missing:
            recommendations.append("ファイルサイズ制限を実装してください")
        
        if 'Secure filename function' in missing:
            recommendations.append("secure_filename()等を使用してファイル名をサニタイズしてください")
        
        if not security_analysis.get('has_path_traversal_protection', False):
            recommendations.append("パストラバーサル攻撃対策を実装してください")
        
        base_recommendation = "ファイルアップロード処理にセキュリティ検証を追加してください。"
    ...
    def _generate_security_report(self, result: FileUploadTestResult):
        ...
        report_data = {
            ...
            "malicious_file_tests": {
                ...
                "passed_tests": sum(1 for passed in result.malicious_file_tests.values() if passed),
                "total_tests": len(result.malicious_file_tests)
            },
            "recommendations": result.recommendations,
            "risk_distribution": self._get_upload_risk_distribution(result.vulnerabilities_found),
            "validation_distribution": self._get_validation_distribution(result.vulnerabilities_found)
        }
    ...
    def _get_upload_risk_distribution(self, vulnerabilities: List[FileUploadVulnerability]) -> Dict[str, int]:
        """ファイルアップロードリスク分布取得"""
        distribution = {}
    ...
    def _get_validation_distribution(self, vulnerabilities: List[FileUploadVulnerability]) -> Dict[str, int]:
        """検証不足分布取得"""
        distribution = {}
        for vuln in vulnerabilities:
...
def main():
    ...
    try:
        with FileUploadTester(project_root) as tester:
            ...
            logger.info(f"  アップロードハンドラー数: {result.total_upload_handlers_found}")
            ...
            logger.info(f"  悪意ファイルテスト: {sum(result.malicious_file_tests.values())}/{len(result.malicious_file_tests)} passed")
            ...
            if critical_vulns:
                logger.warning(f"🚨 高リスクファイルアップロード脆弱性 {len(critical_vulns)}件:")
                for vuln in critical_vulns[:5]:  # 上位5件表示
                    logger.warning(f"  - {vuln.file_path}:{vuln.line_number} ({vuln.risk_level.value})")
                    logger.warning(f"    不足検証: {', '.join(vuln.missing_validations)}")
            
            return 0 if result.overall_risk in [FileUploadRisk.SAFE, FileUploadRisk.LOW] else 1
