...
class MemoryManager:
        ...
        if len(set(object_counts)) > 1:  # 変化がある場合
            ...
            if object_increase > 1000:  # 1000オブジェクト以上増加
                ...
                severity = (
                    "critical"
                    if leak_rate > 100
                    else "high" if leak_rate > 50 else "medium"
                )
    ...
    def _handle_resource_leaks(self, leaks: List[ResourceLeak]):
        ...
        for leak in leaks:
            logger.error(
                f"リソースリーク検出: {leak.resource_type} (+{leak.leak_count}, {leak.leak_rate:.1f}/min)"
            )
    ...
    def get_memory_report(self) -> Dict:
        ...
        report = {
            ...
            "memory_trend": (
                self._calculate_memory_trend(list(self.memory_history)[-10:])
                if len(self.memory_history) >= 10
                else 0
            ),
