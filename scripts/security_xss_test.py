...
class XSSTester(TDDSystemBase):
            ...
            for match in re.finditer(pattern, content, re.IGNORECASE | re.MULTILINE):
                ...
                if not self._is_safe_html_context(content, match):
                    vulnerability = self._create_vulnerability(
                        file_path, line_num, "html_output",
                        description, match.group(), context, XSSRisk.HIGH
                    )
    ...
    def _check_javascript_patterns(self, file_path: Path, content: str):
        ...
        for pattern, description in self.javascript_patterns.items():
            for match in re.finditer(pattern, content, re.IGNORECASE):
                ...
                vulnerability = self._create_vulnerability(
                    file_path, line_num, "javascript_execution",
                    description, match.group(), "javascript", XSSRisk.HIGH
                )
                self.vulnerabilities.append(vulnerability)
    ...
    def _check_url_patterns(self, file_path: Path, content: str):
        ...
        for pattern, description in self.url_patterns.items():
            for match in re.finditer(pattern, content, re.IGNORECASE):
                ...
                vulnerability = self._create_vulnerability(
                    file_path, line_num, "url_construction",
                    description, match.group(), "url", XSSRisk.MEDIUM
                )
                self.vulnerabilities.append(vulnerability)
    ...
    def _check_template_variables(self, file_path: Path, content: str):
        ...
        for match in re.finditer(jinja_pattern, content):
            ...
            if not self._is_escaped_variable(variable):
                vulnerability = self._create_vulnerability(
                    file_path, line_num, "template_variable",
                    f"Potentially unescaped template variable: {variable}",
                    match.group(), "html", XSSRisk.MEDIUM
                )
                self.vulnerabilities.append(vulnerability)
                self.outputs_found += 1
    ...
    def _check_inline_javascript(self, file_path: Path, content: str):
        ...
        for match in re.finditer(script_pattern, content, re.DOTALL | re.IGNORECASE):
            ...
            if re.search(r'\{\{[^}]+\}\}|\{%[^%]+%\}', script_content):
                vulnerability = self._create_vulnerability(
                    file_path, line_num, "inline_javascript",
                    "Dynamic content in inline JavaScript",
                    match.group()[:100], "javascript", XSSRisk.HIGH
                )
                self.vulnerabilities.append(vulnerability)
    ...
    def _check_dangerous_attributes(self, file_path: Path, content: str):
        ...
        for match in re.finditer(event_pattern, content, re.IGNORECASE):
            ...
            vulnerability = self._create_vulnerability(
                file_path, line_num, "dangerous_attribute",
                f"Dynamic content in {event_name} attribute",
                match.group(), "javascript", XSSRisk.HIGH
            )
            self.vulnerabilities.append(vulnerability)
    ...
    def _create_vulnerability(self, file_path: Path, line_number: int,
                            ...
                            code_snippet: str, context: str,
                            default_risk: XSSRisk) -> XSSVulnerability:
        """脆弱性オブジェクト作成"""
        
        # リスクレベル評価
        risk_level, severity_score = self._assess_xss_risk(description, code_snippet, context)
    ...
    def _assess_xss_risk(self, description: str, code_snippet: str, context: str) -> Tuple[XSSRisk, float]:
        """XSSリスクレベル評価"""
        ...
        for keyword in critical_keywords:
            if re.search(keyword.lower(), desc_lower) or re.search(keyword.lower(), code_lower):
    ...
    def _generate_xss_recommendation(self, description: str, context: str) -> str:
        ...
        base_recommendation = context_recommendations.get(context, 
    ...
    def _generate_security_report(self, result: XSSTestResult):
        ...
        report_data = {
            ...
            "risk_distribution": self._get_xss_risk_distribution(result.vulnerabilities_found),
            "context_distribution": self._get_context_distribution(result.vulnerabilities_found)
        }
    ...
    def _get_xss_risk_distribution(self, vulnerabilities: List[XSSVulnerability]) -> Dict[str, int]:
        """XSSリスク分布取得"""
        distribution = {}
    ...
    def _get_context_distribution(self, vulnerabilities: List[XSSVulnerability]) -> Dict[str, int]:
        """コンテキスト分布取得"""
        distribution = {}
        for vuln in vulnerabilities:
            context = vuln.context_type
        ...
        with XSSTester(project_root) as tester:
            ...
            if critical_vulns:
                ...
                for vuln in critical_vulns[:5]:  # 上位5件表示
                    logger.warning(f"  - {vuln.file_path}:{vuln.line_number} ({vuln.risk_level.value}, {vuln.context_type})")
