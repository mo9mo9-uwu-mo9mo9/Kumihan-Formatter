...
class RealTimeQualityMonitor:
    ...
    def _quality_analyzer(self):
        ...
        while self.running:
            try:
                ...
                if (
                    has_changes
                    or time_since_last >= self.monitor_config["quality_check_interval"]
                ):
    ...
    def _create_quality_snapshot(self) -> Optional[QualitySnapshot]:
        ...
        try:
            ...
            logger.debug(
                f"スナップショット作成完了: カバレッジ{coverage_percentage:.1f}%"
            )
            return snapshot
    ...
    def _process_quality_snapshot(self, snapshot: QualitySnapshot):
        ...
        cutoff_date = datetime.now() - timedelta(
            days=self.monitor_config["snapshot_retention_days"]
        )
        self.quality_history = [
            s for s in self.quality_history if s.timestamp > cutoff_date
        ]
    ...
    def _generate_alerts(
        self, before: QualitySnapshot, after: QualitySnapshot
    ) -> List[QualityAlert]:
        """アラート生成"""
        alerts = []
        ...
        try:
            if sys.platform == "darwin":
                icon = (
                    ...
                    else "🚨" if alert.severity == "critical" else "📈"
                )
                subprocess.run(
                    [
                        "osascript",
        ...
        status_data = {
            ...
            "coverage": {
                ...
                "status": self._get_coverage_status(
                    self.current_snapshot.coverage_percentage
                ),
            },
            "tests": {
                "total": self.current_snapshot.test_count,
                "failed": self.current_snapshot.failed_tests,
                "passed": self.current_snapshot.test_count
                - self.current_snapshot.failed_tests,
            },
            "quality": {
    ...
    def _calculate_quality_score(
        self, coverage: float, test_count: int, failed_tests: int, complexity: float
    ) -> float:
        """品質スコア計算"""
    ...
    def _detect_quality_violations(
        self, coverage: float, complexity: float, failed_tests: int
    ) -> List[str]:
        """品質違反検出"""
        violations = []
    ...
    def _create_initial_snapshot(self):
        ...
        if snapshot:
            ...
            self._save_snapshots()
            logger.info(
                f"✅ 初期スナップショット作成完了: カバレッジ{snapshot.coverage_percentage:.1f}%"
            )
        else:
