...
class PhaseIntegrationTester:
    ...
    def generate_report(self):
        ...
        total_passed = sum(
            result.passed_tests for result in self.phase_results.values()
        )
        total_failed = sum(
            result.failed_tests for result in self.phase_results.values()
        )
        ...
        for phase_name, result in self.phase_results.items():
            logger.info(
                f"  {phase_name}: {result.success_rate:.1f}% ({result.passed_tests}/{result.test_count})"
            )
