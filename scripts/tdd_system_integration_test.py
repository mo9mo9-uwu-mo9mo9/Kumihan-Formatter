...
from tdd_system_base import TDDSystemBase, TDDSystemConfig, create_tdd_config, TDDSystemRegistry
...
class TDDSystemIntegrationTest:
    ...
    def _setup_test_environment(self):
        ...
        # 必要なディレクトリ構成作成
        test_dirs = [
            "scripts", ".tdd_logs", ".quality_data", 
            "tests", "kumihan_formatter/core"
        ]
        ...
        # 必要なファイルコピー
        script_files = [
            "secure_subprocess.py", "resource_manager.py", 
            "performance_optimizer.py", "tdd_system_base.py"
        ]
    ...
    def _test_realtime_monitoring_integration(self) -> Dict:
        ...
        vscode_status_file = self.test_project_root / ".quality_data" / "vscode_status.json"
    ...
    def _test_scalability(self) -> Dict:
        ...
        # スケーラビリティ検証
        average_throughput = sum(pt["throughput"] for pt in processing_times) / len(processing_times)
        scalability_good = average_throughput > 1000  # 1000 items/sec 以上
        
        return {
            "processing_times": processing_times,
            "average_throughput": average_throughput,
            "scalability_assessment": "good" if scalability_good else "needs_improvement"
        }
