...
class DependencyIntegrityChecker(TDDSystemBase):
    ...
    def __init__(self, project_root: Path):
        ...
        # requests依存関係チェック - 脆弱性チェック機能の制限事項を明確化
        if not HAS_REQUESTS:
            logger.warning(
                "requests not available - online vulnerability checking disabled"
            )
            logger.info(
                "Install requests package for enhanced vulnerability checking: pip install requests"
            )
            # 高度な脆弱性チェック機能を無効化するフラグ
            self._vulnerability_checking_enabled = False
        else:
            self._vulnerability_checking_enabled = True
    ...
    def initialize(self) -> bool:
        ...
        if not found_configs:
            logger.error(
                "プロジェクト設定ファイルが見つかりません (pyproject.toml, requirements.txt)"
            )
            return False
    ...
    def execute_main_operation(self) -> DependencyTestResult:
        ...
        try:
            ...
            logger.info(
                f"✅ 依存関係整合性チェック完了: {len(self.security_issues + self.license_issues + self.version_conflicts)}件の問題発見"
            )
            return result
            ...
            try:
                ...
                sanitized_error = safe_handle_exception(
                    e,
                    context={
                        "operation": "dependency_integrity_check",
                ...
                logger.error(
                    f"依存関係チェック実行エラー [{sanitized_error.trace_id}]: {sanitized_error.user_message}"
                )
                raise TDDSystemError(
                    f"チェック実行失敗 [{sanitized_error.error_code}]: {sanitized_error.user_message}"
                )
            except ImportError:
    ...
    def _check_security_vulnerabilities(self):
        ...
        if self._vulnerability_checking_enabled and HAS_REQUESTS:
    ...
    def _advanced_vulnerability_check(self):
        """高度な脆弱性チェック（requests利用）"""
        # requests が利用可能かチェック - 二重チェックによる安全性確保
        if (
            not HAS_REQUESTS
            or requests is None
            or not self._vulnerability_checking_enabled
        ):
            logger.info(
                "requests未利用または未インストール - 高度な脆弱性チェックをスキップ"
            )
            return
    ...
    def _assess_vulnerability_severity(self, vuln: Dict) -> DependencyRisk:
        ...
        if any(
            word in advisory for word in ["remote code execution", "rce", "critical"]
        ):
            return DependencyRisk.CRITICAL
        ...
        for pkg_name, pkg_info in self.dependency_tree.items():
            if pkg_info.license:
                ...
                # 互換性チェック
                    ...
                    pkg_license not in compatible_licenses
                    and pkg_license != project_license
                ):
                    issue = DependencyIssue(
                        package_name=pkg_name,
                        current_version=pkg_info.version,
    ...
    def _check_unused_dependencies(self):
        ...
        try:
            ...
            direct_deps = [
                name
                for name, info in self.dependency_tree.items()
                ...
                for pkg in outdated_packages:
                    ...
                    if pkg_name in self.dependency_tree:
                        issue = DependencyIssue(
                            ...
                            severity=self._assess_outdated_severity(
    ...
    def _analyze_results(self, start_time: datetime) -> DependencyTestResult:
        ...
        direct_deps = sum(
            1 for info in self.dependency_tree.values() if info.is_direct_dependency
        )
    ...
    def _generate_dependency_recommendations(self) -> List[str]:
        ...
        if self.unused_deps:
            recommendations.append(
                f"{len(self.unused_deps)}個の未使用依存関係を削除する"
            )
    ...
    def _get_dependency_risk_distribution(
...
def main():
    ...
    try:
        with DependencyIntegrityChecker(project_root) as checker:
            ...
            critical_issues = [
                i
                for i in result.security_issues + result.license_issues
            ...
            if critical_issues:
                ...
                for issue in critical_issues[:5]:  # 上位5件表示
                    logger.warning(
                        f"  - {issue.package_name} ({issue.severity.value}): {issue.description}"
                    )

            if result.unused_dependencies:
                logger.info(
                    f"🗑️ 未使用依存関係: {', '.join(result.unused_dependencies[:5])}"
                )
            ...
            return (
                0
                if result.overall_risk in [DependencyRisk.SAFE, DependencyRisk.LOW]
                else 1
            )
