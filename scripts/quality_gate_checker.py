...
class QualityGateChecker:
    ...
    def _check_module_quality(self, module_name: str, tier: TierLevel, config: Dict) -> QualityGateResult:
        """個別モジュールの品質チェック"""
        ...
        if complexity_score > max_complexity:
            violations.append(
                f"複雑度超過: {complexity_score:.1f} > {max_complexity}"
            )
    ...
    def generate_quality_report(self, results: List[QualityGateResult]) -> str:
        ...
        for tier, summary in tier_summary.items():
            ...
            report.append(f"  {status} {tier.title()} Tier: {summary['passed']}/{summary['total']} ({passed_rate:.1f}%)")
        ...
        for result in results:
            ...
            report.append(f"    カバレッジ: {result.coverage_percentage:.1f}% (要求: {result.required_coverage}%)")
            report.append(f"    複雑度: {result.complexity_score:.1f} (上限: {result.max_complexity})")
        ...
        if failed_results:
            ...
            critical_failures = [r for r in failed_results if r.tier == TierLevel.CRITICAL]
