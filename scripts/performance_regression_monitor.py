...
class PerformanceRegressionMonitor(TDDSystemBase):
    ...
    def initialize(self) -> bool:
        ...
        if memory.available < 512 * 1024 * 1024:  # 512MB未満
            logger.warning(
                f"使用可能メモリが少ない: {memory.available / (1024**2):.1f}MB"
            )
    ...
    def execute_main_operation(self) -> PerformanceTestResult:
        ...
        try:
            ...
            logger.info(
                f"✅ パフォーマンス回帰監視完了: {len(self.regressions)}件の回帰、{len(self.memory_leaks)}件のメモリリーク発見"
            )
            return result
    ...
    def _load_performance_baselines(self):
        ...
        if self.baseline_file.exists():
            try:
                ...
                logger.info(
                    f"ベースライン読み込み完了: {len(self.performance_baselines)}テスト"
                )
            except Exception as e:
    ...
    def _run_benchmark_tests(self):
        ...
        for i, benchmark_test in enumerate(self.benchmark_tests, 1):
            ...
            try:
                ...
                if result:
                    ...
                    logger.debug(
                        f"✅ {test_name} 完了: {duration:.3f}s, {result.memory_usage/1024/1024:.1f}MB"
                    )
                else:
        ...
        logger.info(
            f"ベンチマークテスト完了: {len(self.benchmark_results)}/{total_tests}"
        )
    ...
    def _calculate_regression(
        self, metric_name: str, current: float, baseline: float, test_name: str
        ...
        if is_regression:
            ...
            return PerformanceRegression(
                ...
                confidence_level=self._calculate_confidence_for_regression(
    ...
    def _calculate_confidence_for_regression(
        ...
        if (
            test_name in self.performance_baselines
            and metric_name in self.performance_baselines[test_name]
        ):
    ...
    def _generate_performance_report(self, result: PerformanceTestResult):
        ...
        report_data = {
            ...
            "risk_distribution": self._get_performance_risk_distribution(
                result.regressions
            ),
        }
    ...
    def _get_performance_risk_distribution(
...
def main():
    ...
    try:
        with PerformanceRegressionMonitor(project_root) as monitor:
            ...
            logger.info(
                f"  メモリ: {result.system_info['memory_total'] / (1024**3):.1f}GB"
            )
            ...
            if result.benchmark_results:
                ...
                for bench in result.benchmark_results:
                    logger.info(
                        f"  {bench.test_name}: {bench.execution_time:.3f}s, {bench.memory_usage/1024/1024:.1f}MB"
                    )
            ...
            if critical_regressions:
                logger.warning(
                    f"🚨 高リスクパフォーマンス回帰 {len(critical_regressions)}件:"
                )
                for reg in critical_regressions[:5]:  # 上位5件表示
                    logger.warning(
                        f"  - {reg.test_name}.{reg.metric_name}: {reg.regression_percentage:+.1f}% ({reg.severity.value})"
                    )
            ...
            if result.memory_leaks:
                ...
                for leak in result.memory_leaks:
                    logger.warning(
                        f"  - {leak.function_name}: {leak.leaked_memory/1024/1024:.1f}MB リーク"
                    )

            return (
                0
                if result.overall_risk in [PerformanceRisk.SAFE, PerformanceRisk.LOW]
                else 1
            )
