...
class CrossPlatformIntegrationTester(TDDSystemBase):
    ...
    def initialize(self) -> bool:
        ...
        # プラットフォーム情報表示
        logger.info(f"実行環境: {self.platform_info['system']} {self.platform_info['platform']}")
        logger.info(f"Python: {self.platform_info['python_version']} ({self.platform_info['python_implementation']})")
    ...
    def execute_main_operation(self) -> PlatformTestResult:
        ...
        try:
            ...
            logger.info(f"✅ クロスプラットフォーム統合テスト完了: {len(self.platform_issues)}件の問題発見")
            return result
        ...
        for i, test_case in enumerate(self.test_cases, 1):
            ...
            try:
                ...
                if result['passed']:
                    ...
                    failed += 1
                    logger.warning(f"❌ {test_name} 失敗: {result.get('error', 'Unknown error')}")
                    ...
                    issue = PlatformIssue(
                        ...
                        file_path=None,
                        line_number=None,
                        recommendation=result.get('recommendation', 'テスト失敗の原因を調査してください'),
                        test_case=test_name,
    ...
    def _test_file_operations(self) -> Dict:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                return {'passed': True, 'details': {'platform': self.platform_info['system']}}
    ...
    def _test_process_execution(self) -> Dict:
        ...
        try:
            ...
            if self.platform_info['system'] == 'Windows':
                cmd_result = subprocess.run(['echo', 'test'], capture_output=True, text=True)
            else:
                cmd_result = subprocess.run(['echo', 'test'], capture_output=True, text=True)
    ...
    def _test_dependency_compatibility(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'error': f'インポート失敗: {failed_imports}' if failed_imports else None
            }
        ...
        try:
            ...
            return {
                ...
                'metrics': {
                    ...
                    'write_speed_mb_per_sec': (file_size / (1024 * 1024)) / write_duration,
                    'read_speed_mb_per_sec': (file_size / (1024 * 1024)) / read_duration
                }
            }
    ...
    def _generate_compatibility_report(self, result: PlatformTestResult):
        ...
        report_data = {
            ...
            "test_summary": {
                ...
                "success_rate": (result.passed_tests / result.total_tests_run * 100) if result.total_tests_run > 0 else 0,
...
def main():
    ...
    try:
        with CrossPlatformIntegrationTester(project_root) as tester:
            ...
            logger.info(f"  実行環境: {result.platform_info['system']} {result.platform_info['platform']}")
            logger.info(f"  総テスト数: {result.total_tests_run}")
            logger.info(f"  成功: {result.passed_tests}, 失敗: {result.failed_tests}")
            logger.info(f"  成功率: {(result.passed_tests / result.total_tests_run * 100):.1f}%")
            ...
            # 重要な問題を表示
            critical_issues = [i for i in result.platform_issues 
                             if i.severity in [PlatformCompatibilityRisk.CRITICAL, PlatformCompatibilityRisk.HIGH]]
            
            if critical_issues:
                logger.warning(f"🚨 高リスクプラットフォーム問題 {len(critical_issues)}件:")
                for issue in critical_issues[:5]:  # 上位5件表示
                    location = f"{issue.file_path}:{issue.line_number}" if issue.file_path else issue.test_case
                    logger.warning(f"  - {location} ({issue.severity.value}): {issue.description}")
            
            return 0 if result.overall_risk in [PlatformCompatibilityRisk.SAFE, PlatformCompatibilityRisk.LOW] else 1
