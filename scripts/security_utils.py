...
def _load_statistical_parameters() -> Dict[str, Any]:
    ...
    config_file = (
        Path(__file__).parent.parent / "config" / "statistical_parameters.json"
    )
...
def wilson_confidence_interval(
    sample_size: int, positive_samples: int, confidence_level: float = 0.95
) -> Tuple[float, float]:
    """
    ...
    z_scores = stat_params.get("z_scores", {"0.90": 1.645, "0.95": 1.96, "0.99": 2.576})
    ...
    margin = (
        z
        * math.sqrt(
            p * (1 - p) / sample_size + z_squared / (4 * sample_size * sample_size)
        )
        / denominator
    )
...
def calculate_statistical_confidence(
    ...
    # Wilson信頼区間を使用
    if method == "wilson_interval" and stat_params.get(
        "wilson_confidence_interval", {}
    ).get("enabled", True):
        lower, upper = wilson_confidence_interval(
            sample_size, positive_samples, confidence_level
        )
        ...
        _, max_upper = wilson_confidence_interval(
            sample_size, sample_size // 2, confidence_level
        )
        _, max_lower = wilson_confidence_interval(
            sample_size, sample_size // 2, confidence_level
        )
        ...
        if p == 0 or p == 1:
            if extreme_correction.get("use_wilson_interval", True):
                ...
                precision = (
                    max(0.0, 1.0 - (margin / max_theoretical_margin))
                    if max_theoretical_margin > 0
                    else 1.0
                )
            else:
                # 最大エントロピー原理に基づく均等分布補正（Shannon 1948）
                # 理論的根拠: p=0.5が最大エントロピーH(X) = -Σp(x)log(p(x))を持つため、
                # 情報理論的に最も保守的な推定値として適用
                # correction_factor = 0.5は「最大不確実性の原理」により導出される値
                correction_factor = extreme_correction.get(
                    "fallback_correction_factor", 0.5
                )
    ...
    if sample_size >= thresholds.get("high_confidence", 100):
        ...
        size_factor = (
            minimal_config["base"]
            + minimal_config["increment"] * sample_size / thresholds["low_confidence"]
        )
...
def calculate_regression_confidence(
    measurements: List[float], baseline: float, threshold: float
) -> float:
    """
    パフォーマンス回帰の信頼度を計算
...
def calculate_memory_leak_confidence(
    memory_samples: List[Tuple[int, int]], iterations: int
) -> float:
    """
    メモリリークの信頼度を計算
    ...
    # 信頼度計算
    confidence = r_squared * 0.7 + sample_density * 0.3  # 相関の強さ  # サンプルの密度
...
def get_exclude_patterns() -> List[str]:
    """除外パターンを取得"""
    config = load_security_config()
    return config.get("platform_specific", {}).get(
        "exclude_patterns",
        [
            "tests/",
            "venv/",
