    ...
    def execute(
        ...
        try:
            ...
            if subprocess_result.success:
                logger.debug(
                    f"コマンド実行成功 ({execution_time:.2f}s): {safe_command[0]}"
                )
            else:
                logger.warning(
                    f"コマンド実行失敗 (code:{result.returncode}): {safe_command[0]}"
                )
                if result.stderr:
            ...
            logger.error(
                f"コマンドタイムアウト ({execution_timeout}s): {safe_command[0]}"
            )
    ...
    def _check_concurrent_limit(self) -> None:
        ...
        with self._lock:
            if len(self._running_processes) >= self.max_concurrent:
                raise ResourceError(
                    f"同時実行数上限 ({self.max_concurrent}) に達しました"
                )
