...
class TDDSessionManager:
    ...
    def start_session(self, issue_number: str) -> TDDSession:
        ...
        if self.session_file.exists():
            ...
            if existing_session:
                logger.warning(f"既存セッションが存在します: Issue #{existing_session.issue_number}")
                response = input("既存セッションを終了して新しいセッションを開始しますか? (y/N): ")
                if response.lower() != 'y':
    ...
    def _fetch_issue_info(self, issue_number: str) -> Dict:
        ...
        try:
            ...
            cmd = ["gh", "issue", "view", issue_number, "--json", "title,body,labels,assignees"]
    ...
    def _ensure_correct_branch(self, issue_number: str, issue_title: str) -> str:
        ...
        # 期待するブランチ名生成
        safe_title = "".join(c for c in issue_title.lower() if c.isalnum() or c in '-_')[:50]
...
def main():
    ...
    parser.add_argument("command", choices=["start", "status", "debug"],
                       help="実行コマンド")
    ...
    if args.command == "start":
        ...
        if status["active"]:
            ...
            print(f"📊 カバレッジ: {status['current_metrics']['coverage_percentage']:.1f}%")
        else:
        ...
        if session:
            print(json.dumps(asdict(session), indent=2, default=str, ensure_ascii=False))
        else:
