...
class SQLInjectionTester(TDDSystemBase):
    ...
    def __init__(self, project_root: Path):
        ...
        if patterns and "sql_injection" in patterns:
            ...
            logger.info(
                "Using default SQL injection patterns (configuration not available)"
            )
    ...
    def execute_main_operation(self) -> SQLInjectionTestResult:
        ...
        try:
            ...
            logger.info(
                f"✅ SQLインジェクションテスト完了: {len(self.vulnerabilities)}件の脆弱性発見"
            )
            return result
    ...
    def _scan_source_files(self):
        ...
        logger.info(
            f"📊 スキャン完了: {self.scanned_files}ファイル、{self.scanned_functions}関数"
        )
    ...
    def _assess_risk(
        self, description: str, code_snippet: str
    ) -> Tuple[SQLInjectionRisk, float]:
        """リスクレベル評価"""
        # キーワードベースのリスク評価
...
def main():
    ...
    try:
        with SQLInjectionTester(project_root) as tester:
            ...
            return (
                0
                if result.overall_risk in [SQLInjectionRisk.SAFE, SQLInjectionRisk.LOW]
                else 1
            )
