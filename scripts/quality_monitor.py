...
class QualityMonitor:
    ...
    def _count_lines_of_code(self) -> Dict:
        ...
        return {
            ...
            "comment_ratio": (
                ((total_lines - code_lines) / total_lines * 100)
                if total_lines > 0
                else 0
            ),
        }
    ...
    def _analyze_imports(self) -> Dict:
        ...
        try:
            result = subprocess.run(
                ["grep", "-r", "^import\\|^from", str(self.src_dir), "--include=*.py"],
                capture_output=True,
                text=True,
            )
    ...
    def _analyze_complexity(self) -> Dict:
        ...
        return {
            ...
            "avg_functions_per_file": round(
                total_functions / max(1, len(list(self.src_dir.glob("**/*.py")))), 1
            ),
        }
                ...
                for part in parts:
                    if "%" in part:
                        ...
                        return {
                            ...
                            "target_compliance": coverage
                            >= self.targets["test_coverage"],
                        }
    ...
    def _measure_test_duration(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "performance_rating": (
                    "fast" if duration < 10 else "moderate" if duration < 30 else "slow"
                ),
            }
        except Exception:
    ...
    def _measure_build_time(self) -> Dict:
        ...
        try:
            start_time = time.time()
            result = subprocess.run(
                ["make", "lint"], capture_output=True, text=True, cwd=self.project_dir
            )
            duration = time.time() - start_time
    ...
    def _measure_lint_time(self) -> Dict:
        ...
        try:
            start_time = time.time()
            subprocess.run(
                ["python3", "-m", "black", "--check", str(self.src_dir)],
                capture_output=True,
                cwd=self.project_dir,
            )
    ...
    def _estimate_memory_usage(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "avg_file_size_kb": (
                    round((total_size / file_count) / 1024, 1) if file_count > 0 else 0
                ),
            }
        except Exception:
    ...
    def _calculate_quality_score(
        self, code_metrics: Dict, test_metrics: Dict, perf_metrics: Dict
    ) -> Dict:
        """品質スコア計算"""
        score = 0
        max_score = 0
    ...
    def _generate_recommendations(
        ...
        if mypy_errors > self.targets["mypy_errors"]:
            recommendations.append(
                f"mypy エラーを {mypy_errors - self.targets['mypy_errors']} 個削減してください"
            )

        imports = code_metrics.get("imports", {}).get("total_imports", 0)
        if imports > self.targets["import_count"]:
            recommendations.append(
                f"import文を {imports - self.targets['import_count']} 個削減してください"
            )
        ...
        if coverage < self.targets["test_coverage"]:
            recommendations.append(
                f"テストカバレッジを {self.targets['test_coverage'] - coverage}% 向上してください"
            )
        ...
        if build_time > self.targets["build_time"]:
            recommendations.append(
                f"ビルド時間を {build_time - self.targets['build_time']} 秒短縮してください"
            )
    ...
    def print_summary(self, report: Dict):
        ...
        print(
            f"📊 総合品質スコア: {quality_score['score']}/{quality_score['max_score']} ({quality_score['percentage']}%) - グレード{quality_score['grade']}"
        )
        ...
        print(
            f"  🔍 mypy エラー: {code.get('mypy_results', {}).get('error_count', 'N/A')} (目標: ≤{self.targets['mypy_errors']})"
        )
        print(
            f"  📦 import文: {code.get('imports', {}).get('total_imports', 'N/A')} (目標: ≤{self.targets['import_count']})"
        )
        print(
            f"  🧪 テストカバレッジ: {test.get('coverage', {}).get('coverage_percent', 'N/A')}% (目標: ≥{self.targets['test_coverage']}%)"
        )
        print(
            f"  ⚡ ビルド時間: {perf.get('build_time', {}).get('duration_seconds', 'N/A')}秒 (目標: ≤{self.targets['build_time']}秒)"
        )
...
def main():
    ...
    parser.add_argument(
        "--code", action="store_true", help="コード品質メトリクスのみ収集"
    )
    parser.add_argument(
        "--test", action="store_true", help="テスト品質メトリクスのみ収集"
    )
    parser.add_argument(
        "--perf", action="store_true", help="パフォーマンスメトリクスのみ収集"
    )
    parser.add_argument(
        "--full", action="store_true", help="全メトリクス収集（デフォルト）"
    )
